...
def main():
    ...
    wsz_census_gb = (
        wsz_census.groupby(["WSZONEID"])["TOTAL_POP"].sum().to_frame().reset_index()
    )
    wsz_census_gb["par"] = wsz_census_gb["WSZONEID"].str[:3]

    parish_names_df = pd.DataFrame(
        {
            "par": [
                "ann",
                "eli",
    ...
    wsz_census_gb_par = wsz_census_gb.merge(
        parish_stats[
            [
                "PARISH",
                "% RESIDENTIAL CONNECTED 2010",
                "Leakage rate 2010 ",
                "Supply 2010",
                "Population 2010",
                "Population 2030",
    ...
    wsz_census_gb_par["Population 2030"] = pd.to_numeric(
        wsz_census_gb_par["Population 2030"], errors="coerce"
    )
    wsz_census_gb_par["Population 2010"] = pd.to_numeric(
        wsz_census_gb_par["Population 2010"], errors="coerce"
    )
    wsz_census_gb_par["Growth"] = (
        wsz_census_gb_par["Population 2030"] / wsz_census_gb_par["Population 2010"]
    )
    wsz_census_gb_par["system_connected_pop_2010"] = (
        wsz_census_gb_par["TOTAL_POP"]
        * wsz_census_gb_par["% RESIDENTIAL CONNECTED 2010"]
        / 100
    )
    wsz_census_gb_par["system_connected_pop_2030"] = (
        wsz_census_gb_par["system_connected_pop_2010"] * wsz_census_gb_par["Growth"]
    )
    ...
    wsz_potable_assets = pd.read_csv(
        "potable/processed/wsz_potable_supply_assets_3.csv"
    )
    wsz_potable_assets["OBJECTID"] = pd.to_numeric(wsz_potable_assets["OBJECTID"])
    wsz_potable_assets["node_id"] = (
        wsz_potable_assets["Type"].astype(str)
        + "_"
        + wsz_potable_assets["OBJECTID"].astype(float).astype(str)
    )

    wsz_potable_assets_merge = wsz_potable_assets.merge(
        wsz_census_gb_par, on="WSZONEID", how="left"
    ).drop_duplicates()
    wsz_potable_assets_merge["Type"] = np.where(
        wsz_potable_assets_merge["LOCATION"].isin(["Mona Dam", "Hermitage Dam"]),
        "dam",
        wsz_potable_assets_merge["Type"],
    )
    ...
    wsz_potable_assets_merge["node_type"] = np.where(
        wsz_potable_assets_merge["Type"].isin(source),
        "source",
        np.where(
            wsz_potable_assets_merge["Type"].isin(junction),
            "junction",
            np.where(wsz_potable_assets_merge["Type"].isin(end), "end", "nan"),
        ),
    )
    wsz_potable_assets_merge_gb = (
        wsz_potable_assets_merge.groupby(["WSZONEID", "node_type"])["LOCATION"]
        .count()
        .to_frame()
        .reset_index()
    )
    wsz_potable_assets_merge_gb_end = wsz_potable_assets_merge_gb[
        wsz_potable_assets_merge_gb["node_type"] == "end"
    ]
    wsz_potable_assets_merge_gb_end = wsz_potable_assets_merge_gb_end.rename(
        columns={"LOCATION": "no. ends"}
    )

    potable_assets = wsz_potable_assets_merge.merge(
        wsz_potable_assets_merge_gb_end[["WSZONEID", "no. ends"]],
        on="WSZONEID",
        how="left",
    ).drop_duplicates()
    potable_assets["frac_pop"] = np.where(
        potable_assets["Type"].isin(source),
        1,
        np.where(
            potable_assets["Type"].isin(junction),
            1,
            np.where(
                potable_assets["Type"].isin(end), 1 / potable_assets["no. ends"], 0
            ),
        ),
    )
    potable_assets["asset_connected_pop_2010"] = (
        potable_assets["frac_pop"] * potable_assets["system_connected_pop_2010"]
    )
    potable_assets["asset_connected_pop_2030"] = (
        potable_assets["frac_pop"] * potable_assets["system_connected_pop_2030"]
    )
    ...
    potable_facilities_NWC_pop = potable_facilities_NWC.merge(
        potable_assets[["node_id", "WSZONEID", "asset_connected_pop_2010"]],
        on="node_id",
        how="left",
    ).drop_duplicates()
    ...
    pipelines_centroid["edge_id"] = "potable_" + pipelines_centroid["OBJECTID"].astype(
        float
    ).astype(str)
    for it, row in pipelines_centroid.iterrows():  # canal/pipe centroid
        ...
        potable_assets["sqrt"] = np.sqrt(
            np.square(potable_assets["lat_diff"])
            + np.square(potable_assets["lat_diff"])
        )
        closest_wrz = potable_assets[
            potable_assets["sqrt"] == potable_assets["sqrt"].min()
        ]["WSZONEID"].values[0]
        asset_connected_pop_2010 = potable_assets[
            potable_assets["sqrt"] == potable_assets["sqrt"].min()
        ]["asset_connected_pop_2010"].values[0]
        asset_connected_pop_2030 = potable_assets[
            potable_assets["sqrt"] == potable_assets["sqrt"].min()
    ...
    pipelines_centroids["edge_id"] = "pipeline_" + pipelines_centroids[
        "OBJECTID"
    ...
    wells["DXF_TEXT"] = np.where(
        wells["DISTRICT"] == "MID - CLARENDON",
        "MID CLARENDON",
    ...
    wells_merge_gb = (
        wells_merge.groupby(["DXF_TEXT", "ACRES"])["NAME"]
        .count()
        .to_frame()
        .reset_index()
    )
    wells_merge_gb = wells_merge_gb.rename(columns={"NAME": "no. wells"})
    wells_merge_2 = wells_merge.merge(
        wells_merge_gb, on=["DXF_TEXT", "ACRES"], how="left"
    ).drop_duplicates()
    wells_merge_2["acres_per_node"] = (
        wells_merge_2["ACRES"] / wells_merge_2["no. wells"]
    )
    ...
    irrigation_assets_NIC_are = irrigation_assets_NIC.merge(
        wells_merge_2[["node_id", "acres_per_node"]], on="node_id", how="left"
    ).drop_duplicates()
    irrigation_assets_NIC_are.to_file(
        "irrigation_assets_NIC_are_nodes.gpkg", driver="GPKG"
    )
    ...
    for it, row in pipelines_centroids.iterrows():
        ...
        scheme_centroids["sqrt"] = np.sqrt(
            np.square(scheme_centroids["lat_diff"])
            + np.square(scheme_centroids["lat_diff"])
        )
        closest_scheme = scheme_centroids[
            scheme_centroids["sqrt"] == scheme_centroids["sqrt"].min()
        ]["DXF_TEXT"].values[0]
        pipelines_centroids.loc[it, "DXF_TEXT"] = closest_scheme

    pipelines_centroids_merge = pd.merge(
        pipelines_centroids, schemes[["DXF_TEXT", "ACRES"]], on="DXF_TEXT", how="left"
    ).drop_duplicates()
    pipelines_centroids_merge_gb = (
        pipelines_centroids_merge.groupby(["DXF_TEXT", "ACRES"])["Size"]
        .sum()
        .to_frame()
        .reset_index()
    )
    pipelines_centroids_merge_gb = pipelines_centroids_merge_gb.rename(
        columns={"Size": "sum of pipe size"}
    )
    pipelines_centroids_merge_2 = pd.merge(
        pipelines_centroids_merge,
        pipelines_centroids_merge_gb,
        on=["DXF_TEXT", "ACRES"],
        how="left",
    ).drop_duplicates()
    pipelines_centroids_merge_2["acres_per_edge"] = (
        pipelines_centroids_merge_2["ACRES"]
        * pipelines_centroids_merge_2["Size"]
        / pipelines_centroids_merge_2["sum of pipe size"]
    )
    ...
    canals_centroids["DXF_TEXT"] = np.where(
        canals_centroids["Parish"] == "Clarendon",
        "MID CLARENDON",
        canals_centroids["DXF_TEXT"],
    )

    canals_centroids_merge = pd.merge(
        canals_centroids, schemes, on="DXF_TEXT", how="left"
    ).drop_duplicates()
    canals_centroids_merge_gb = (
        canals_centroids_merge.groupby(["DXF_TEXT", "ACRES"])["LENGTH"]
        .sum()
        .to_frame()
        .reset_index()
    )
    canals_centroids_merge_gb = canals_centroids_merge_gb.rename(
        columns={"LENGTH": "sum of canal length"}
    )
    canals_centroids_merge_2 = canals_centroids_merge.merge(
        canals_centroids_merge_gb, on=["DXF_TEXT", "ACRES"], how="left"
    ).drop_duplicates()
    canals_centroids_merge_2["acres_per_edge"] = (
        canals_centroids_merge_2["ACRES"]
        * canals_centroids_merge_2["LENGTH"]
        / canals_centroids_merge_2["sum of canal length"]
    )

    canals_centroids_merge_2["acres_per_edge"] = np.where(
        canals_centroids_merge_2["DXF_TEXT"] == "MID CLARENDON",
        canals_centroids_merge_2["AREA_SERVE"],
        canals_centroids_merge_2["acres_per_edge"],
    )
    ...
    irrigation_assets_NIC_are_egdes = irrigation_assets_NIC.merge(
        edges_centroids[["edge_id", "acres_per_edge", "DXF_TEXT"]],
        on="edge_id",
        how="left",
    ).drop_duplicates()
