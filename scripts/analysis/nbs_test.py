            ...
            if os.path.isfile(hazard_intersection_file) is True:
                ...
                for hazard_info in hazard_attributes.itertuples():
                    if (
                        ...
                        hazard_vals = hazard_data_details[
                        ...
                        mangrove_vals = hazard_data_details[
                            hazard_data_details["hazard"] == "coastal_mangrove"
                        ]
                        nomangrove_vals = hazard_data_details[
                            hazard_data_details["hazard"] == "coastal_nomangrove"
                        ]
                        mangrove_return_periods = [0] + list(
                            sorted(mangrove_vals["rp"].values.tolist())
                        )
                        for ix, (min_rp, max_rp) in enumerate(
                            list(
                                zip(
                                    mangrove_return_periods[:-1],
                                    mangrove_return_periods[1:],
                                )
                            )
                        ):
                            if (
                                len(
                                    hazard_vals[
                                        (hazard_vals["rp"] > min_rp)
                                        & (hazard_vals["rp"] <= max_rp)
                                    ].index
                                )
                                > 0
                            ):
                                hazard_keys = hazard_vals[
                                    (hazard_vals["rp"] > min_rp)
                                    & (hazard_vals["rp"] <= max_rp)
                                ]["key"].values.tolist()
                                mangrove_key = mangrove_vals[
                                    mangrove_vals["rp"] == max_rp
                                ]["key"].values[0]
                                nomangrove_key = nomangrove_vals[
                                    nomangrove_vals["rp"] == max_rp
                                ]["key"].values[0]
                                hazard_df[hazard_keys] = hazard_df[hazard_keys].sub(
                                    hazard_df[nomangrove_key] - hazard_df[mangrove_key],
                                    axis=0,
                                )
                        ...
                        hazard_effect_df = hazard_df[
                            [asset_id, "exposure", "exposure_unit"]
                            + hazard_keys
                            + [nomangrove_key, mangrove_key]
                        ]
