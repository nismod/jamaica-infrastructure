...
def main(config):
    ...
    asset_data_details = pd.read_csv(os.path.join(
                        processed_data_path,
                        "networks",
            ...
            if os.path.isfile(hazard_intersection_file) is True: 
                ...
                for hazard_info in hazard_attributes.itertuples():
                    if getattr(asset_info,f"{hazard_info.hazard}_asset_damage_lookup_column") != 'none':
                        ...
                        hazard_vals = hazard_data_details[hazard_data_details["hazard"] == hazard_info.hazard]
                        hazard_keys = hazard_vals["key"].values.tolist()
                        hazard_df[[asset_id,'exposure','exposure_unit'] + hazard_keys].to_csv(
                                f"{asset_info.asset_gpkg}_{asset_info.asset_layer}_direct_damages_parameter_set_{set_count}_nomangrove.csv",
                                index=False)
                        mangrove_vals = hazard_data_details[hazard_data_details["hazard"] == "coastal_mangrove"]
                        nomangrove_vals = hazard_data_details[hazard_data_details["hazard"] == "coastal_nomangrove"]
                        mangrove_return_periods = [0] + list(sorted(mangrove_vals["rp"].values.tolist()))
                        for ix, (min_rp,max_rp) in enumerate(list(zip(mangrove_return_periods[:-1],mangrove_return_periods[1:]))):
                            if len(hazard_vals[(hazard_vals["rp"] > min_rp) & (hazard_vals["rp"] <= max_rp)].index) > 0:
                                hazard_keys = hazard_vals[(hazard_vals["rp"] > min_rp) & (hazard_vals["rp"] <= max_rp)]["key"].values.tolist()
                                mangrove_key = mangrove_vals[mangrove_vals["rp"] == max_rp]["key"].values[0]
                                nomangrove_key = nomangrove_vals[nomangrove_vals["rp"] == max_rp]["key"].values[0]
                                hazard_df[hazard_keys] = hazard_df[hazard_keys].sub(hazard_df[nomangrove_key] - hazard_df[mangrove_key],axis=0)

                        hazard_keys = hazard_vals["key"].values.tolist()
                        del hazard_vals
                        hazard_effect_df = hazard_df[[asset_id,'exposure','exposure_unit'] + hazard_keys + [nomangrove_key,mangrove_key]]
                        hazard_effect_df.to_csv(
                            f"{asset_info.asset_gpkg}_{asset_info.asset_layer}_direct_damages_parameter_set_{set_count}_mangrove.csv",
                            index=False)
                        ...
                        hazard_effect_df.to_csv(
                            f"{asset_info.asset_gpkg}_{asset_info.asset_layer}_direct_damages_parameter_set_{set_count}_mangrove_threshold.csv",
                            index=False)
                        if len(hazard_effect_df.index) == 0:
                            ...
                            for damage_info in damages_df.itertuples():
                                ...
                                if len(hazard_asset_effect_df.index) > 0:
                                    ...
                                    hazard_asset_effect_df.to_csv(
                                            f"{asset_info.asset_gpkg}_{asset_info.asset_layer}_direct_damages_parameter_set_{set_count}_mangrove_damages.csv",
                                            index=False)
                                    ...
                                    hazard_asset_effect_df.to_csv(
                                            f"{asset_info.asset_gpkg}_{asset_info.asset_layer}_direct_damages_parameter_set_{set_count}_mangrove_damage_costs.csv",
                                            index=False)
