...
def main(config):
    ...
    for root, dirs, files in os.walk(os.path.join(results_dir,"scenario_results")):
        for file in files:
            if file != "single_station_failures_scenarios.csv":
                if file.startswith("single_bridge_failures_scenarios"):
                    ...
                    all_failures.append(df[["edge_id","no_access",
                                            "time_loss","labour_rerouting_loss",
                                            "trade_rerouting_loss","labour_gdp_loss",
                                            "trade_loss","min_trip_time_loss",
                                            "max_trip_time_loss","mean_trip_time_loss"]])
    ...
    labour_flows = gpd.read_file(os.path.join(results_path,
                                    'flow_mapping',
                                    'labour_trips_and_activity.gpkg'),
                            layer='edges')
    ...
    for bridge in bridges:
        bridge_edges = edges[(edges["from_node"] == bridge) | (edges["to_node"] == bridge)] 
    ...
    bridge_labour_trips = pd.read_csv(os.path.join(results_path,
                                    'flow_mapping',
    ...
    #                         bridge_trade_trips,
    #                         how="left",on=["node_id"])
    all_failures = pd.merge(all_failures,
    				labour_flows[["edge_id","working_trips","GDP_to_trips"]],
    				how="left",on=["edge_id"]).fillna(0)
    bridge_roads = pd.merge(bridge_roads,all_failures,
                            how="left",on=["edge_id"])
    ...
    bridge_roads_access = pd.merge(bridge_roads_access,bridge_roads_trips,how="left",on=["node_id","node_degree"])
    ...
    bridge_failures["working_trips"] = (bridge_failures["working_trips_thru"] + bridge_failures["d_trip"] - bridge_failures["o_trip"])/bridge_failures["node_degree"]
    ...
    results_dir = os.path.join(results_path,"economic_losses","single_failure_scenarios")
    all_failures.to_csv(os.path.join(results_dir,"single_point_failure_road_rail_edges_economic_losses.csv"),index=False)
    bridge_failures.to_csv(os.path.join(results_dir,"single_point_failure_road_bridges_economic_losses.csv"),index=False)

    """Get the ports results
    """
    od_losses = pd.read_csv(os.path.join(results_path,
                            "flow_mapping",
                            "origins_destinations_trade_economic_activity.csv"))
    ports = gpd.read_file(os.path.join(processed_data_path,"networks","transport","port_polygon.gpkg"))
    ...
    ports.to_csv(os.path.join(results_dir,"single_point_failure_ports_economic_losses.csv"),index=False)

    """Get rail stations losses
    """
    rail_losses = pd.read_csv(os.path.join(results_path,
                                    "transport_failures",
                                    "scenario_results",
