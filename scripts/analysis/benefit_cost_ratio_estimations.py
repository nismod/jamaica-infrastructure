    ...
    for hazard in adapt_hazards:
        ...
        for asset_info in asset_data_details.itertuples():
            ...
            if (os.path.isfile(cost_file) is True) and (os.path.isfile(risk_file) is True):
                print (f"* Starting with {hazard['hazard']} {asset_info.asset_gpkg} {asset_info.asset_layer}")
                cost_df = pd.read_csv(cost_file)
                adaptation_options = list(set(cost_df["adaptation_option"].values.tolist()))
                no_adapt_risk = pd.read_csv(risk_file)
                no_adapt_risk_df, risk_columns = get_risks(no_adapt_risk,asset_id,hazard['hazard'],hazard['hazard_type'],
                                                            rcps,risk_type,val_type,days=days)
                no_adapt_ead_eael_df, ead_eael_columns = get_ead_eael(no_adapt_risk,asset_id,
                                                        hazard['hazard'],hazard['hazard_type'],
                                                        rcps,risk_type,val_type)
                ...
                for option in adaptation_options:
                    ...
                    if hazard['hazard'] == "flooding" and asset_adaptation_cost == "J$/m":
                        ...
                        ead_eael_results = get_ead_eael_costs(output_data_path,asset_id,ead_eael_results,asset_info,
                                                    hazard,rcps,risk_type,val_type,
                                                    no_adapt_ead_eael_df,ead_eael_columns,
                                                    option_df,
                                                    flood_thresholds,flood_thresholds,"flood_depth_protection_level",
                        ...
                        risk_df.rename(columns=dict(list(zip(risk_columns,adapt_benefit_columns))),inplace=True)
                        bcr_r, bcr_cols = bcr_estimates(asset_id,option_df,
                                                    risk_df,
                                                    "flood_protection_level",
                                                    adapt_benefit_columns)
                        bcr_results.append(bcr_r)
                        
                        adapt_ead_eael_columns = [c.replace("EAD","avoided_EAD").replace("EAEL","avoided_EAEL") for c in risk_columns]
                        ead_eael_df = no_adapt_ead_eael_df.copy()
                        ead_eael_df.rename(columns=dict(list(zip(ead_eael_columns,adapt_ead_eael_columns))),inplace=True)
                        ead_eael_r = ead_eael_estimates(asset_id,option_df,
                                                    ead_eael_df,
                                                    "flood_protection_level",
                ...
                if len(bcr_results) > 0:
                    ...
                    bcr_results.to_csv(os.path.join(adaptation_bcr_results,
                        f"{hazard['hazard']}_{asset_info.asset_gpkg}_{asset_info.asset_layer}_adaptation_benefits_costs_bcr.csv"),index=False)

                    print (f"* Done with {hazard['hazard']} {asset_info.asset_gpkg} {asset_info.asset_layer} BCRs")

                if len(ead_eael_results) > 0:
                    ead_eael_results = pd.concat(ead_eael_results,axis=0,ignore_index=False)

                    ead_eael_results.to_csv(os.path.join(adaptation_bcr_results,
                        f"{hazard['hazard']}_{asset_info.asset_gpkg}_{asset_info.asset_layer}_adaptation_costs_avoided_EAD_EAEL.csv"),index=False)

                    print (f"* Done with {hazard['hazard']} {asset_info.asset_gpkg} {asset_info.asset_layer} EAD EAEL")
