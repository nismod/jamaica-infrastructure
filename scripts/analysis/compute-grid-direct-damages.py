"""Estimate expected annual damages due to tropical cyclone winds (TC) and various types of flooding at grid level.
"""

# TODO: update config.yml as follows
# paths:
#   processed_data: "./processed_data" # /soge-home/projects/mistral/jamaica-ccri/processed_data/damage_curves
#   results: "./results/grid_failures"
#   output: "./results/grid_failures"
...
def main(config):
    ...
    # note processed data & output are currently set to the same path in .yml
    processed_data_path = config["paths"]["results"]
    ...
    asset_data_details = pd.read_csv(
        os.path.join(
            processed_data_path,
            "networks",
            "network_layers_hazard_intersections_details_1.csv",  # TODO: add a column for which contains grid_ids in each asset dataset e.g. jamaica_grid_ids / grid_ids
        )
    )
    ...
    cost_uncertainty_parameter = 0  # TODO: update
    damage_uncertainty_parameter = 0  # TODO: update
    ...
    for idx, hazard in hazard_attributes.iterrows():
        ...
        for asset_info in asset_data_details.itertuples():
            ...
            asset_grid_id = asset_info.asset_grid_id_column
                            ...
                            if len(hazard_effect_df.index) == 0:
                                ...
                                hazard_effect_df = (
                                    hazard_effect_df.groupby(
                                        [
                                            asset_id,
                                            grid_id,  # TODO: incorporate grid id to aggregate by
                                            "exposure_unit",
                                            "damage_cost_unit",
                                            "hazard",
                                            "hazard_threshold",
            ...
            if len(hazard_damages) > 0:
                ...
                hazard_damages.to_csv(
                    os.path.join(
                        ...
                        f"{asset_info.asset_gpkg}_{asset_info.asset_layer}_direct_damages.csv",
                    ),
                    index=False,
                )
...
if __name__ == "__main__":
    CONFIG = load_config()  # note: defined in analysis_utils.py
    main(CONFIG)
