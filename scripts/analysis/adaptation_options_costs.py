...
def main(config):
    ...
    cost_df = pd.read_excel(os.path.join(processed_data_path,
                            "adaptation",
                            "adaptation_options_and_costs_jamaica.xlsx"),sheet_name="Sheet1").fillna(0)
    ...
    for hazard in adapt_hazards:
        ...
        for asset_info in adapt_assets.itertuples():
            ...
            if asset_info.asset_description != "roads":
                ...
                asset_df = pd.merge(asset_df,adapt_costs,how="left",left_on=asset_hazard,right_on="asset_name")
                asset_df = get_adaptation_options_costs(asset_df,asset_id)
            else:
                asset_df = get_adaptation_options_costs_roads(asset_df,adapt_costs,asset_id)
            
            asset_df.to_csv(os.path.join(hazard_outputs,
                        f"{asset_info.asset_gpkg}_{asset_info.asset_layer}_adaptation_unit_costs.csv"),
                        index=False)
            asset_df = assign_costs_over_time(asset_df,asset_id,start_year=baseline_year,
