...
def estimate_time_series(summarised_damages,
						...
						risk_type,val_type,
                        baseline_year,growth_rates,discounting_rate,discounted_values,timeseries_results):
    ...
    defence_type = [c for c in summarised_damages.columns.values.tolist() if "designed_protection" in c]
    ...
    for ix, (haz,rcp) in enumerate(hazard_rcp):
        ...
        df = (haz_rcp_damages.set_index(index_columns).pivot(
                            columns="epoch"
                            )[f"{risk_type}_{defence_type}_{val_type}"].reset_index().rename_axis(None, axis=1)).fillna(0)
