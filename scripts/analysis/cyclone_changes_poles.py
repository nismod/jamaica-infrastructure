...
def main(config):
    ...
    for ft in cyclone_damage_curve_change:
        ...
        networks = networks[(networks["sector"] == "electricity") & (networks["asset_layer"] == "nodes")]
        ...
        damage_curves_csv = os.path.join(results_folder,
                                "asset_damage_curve_mapping.csv")
        ...
        with open("ead_eael_results.txt","w+") as f:
            with open(parameter_combinations_file,"r") as r:
                for p in r:
                    pv = p.split(",")
                    f.write(f"{damage_results_folder},{network_csv},{hazard_csv},{cyclone_protection_column},{pv[0]},{pv[1]},{pv[2]}\n")
