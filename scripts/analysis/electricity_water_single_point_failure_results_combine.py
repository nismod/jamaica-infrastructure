...
def main(config):
    ...
    results_dir = os.path.join(output_data_path,"economic_losses","single_failure_scenarios")
    ...
    buildings_gdp_columns = [c for c in buildings.columns.values.tolist() if "_GDP" in c and c != "total_GDP"]
    ...
    """
    potable_economic_activity_buildings = pd.read_csv(os.path.join(processed_data_path,
    ...
    potable_economic_activity.to_csv(os.path.join(results_dir,
                                    "single_point_failure_potable_facilities_economic_losses.csv"),index=False)
    ...
    """
    pipelines_economic_activity = pd.read_csv(os.path.join(processed_data_path,
                                    "networks_economic_activity",
    ...
    irrigation_economic_activity = pd.read_csv(os.path.join(processed_data_path,
                                    "networks_economic_activity",
    ...
    irrigation_economic_activity.to_csv(os.path.join(results_dir,
                                    "single_point_failure_irrigation_nodes_economic_losses.csv"),index=False)
    print ("* Done with irrigation nodes")
    """Get the Irrigation edge assets with the Agriculture dependent GDP 
    """
    irrigation_edges_economic_activity = pd.read_csv(os.path.join(processed_data_path,
                                    "networks_economic_activity",
    ...
    """
    electricity_economic_activity = pd.read_csv(os.path.join(processed_data_path,
                                    "networks_economic_activity",
                                    "electricity_dependent_economic_activity.csv")) 
    
    gdp_columns = [c for c in electricity_economic_activity.columns.values.tolist() if c not in ("id","total_GDP","GDP_unit")]
    
    electricity_nodes_failure_results = pd.read_csv(os.path.join(
                                    output_data_path,
                                    "electricity_failures",
                                    "single_point_failure_results_nodes.csv"))[["attacked_node_id","affected_node_id"]]
    ...
    failure_duplicates = failure_duplicates.drop_duplicates(subset=["attacked_node_id"],keep="first")
    electricity_nodes_failure_results = pd.concat([electricity_nodes_failure_results,
                                                    failure_duplicates],axis=0,ignore_index=True)
    del failure_duplicates

    electricity_nodes_failures = pd.merge(electricity_nodes_failure_results,
                                            electricity_economic_activity,
                                            how="left",
                                            left_on=["affected_node_id"],
                                            right_on=["id"]).fillna(0)
    electricity_nodes_failures = electricity_nodes_failures.groupby(["attacked_node_id"])[gdp_columns].sum().reset_index()
    electricity_nodes_failures["economic_loss"] = electricity_nodes_failures[gdp_columns].sum(axis=1)
    electricity_nodes_failures["loss_unit"] = "JD/day"
    electricity_nodes_failures.to_csv(os.path.join(results_dir,
                                    "single_point_failure_electricity_nodes_economic_losses_no_water.csv"),index=False)
    electricity_nodes_failures = electricity_nodes_failures[["attacked_node_id","E_GDP","T_GDP"]]
    ...
    """
    electricity_edges_failure_results = pd.read_csv(os.path.join(output_data_path,
                                    "electricity_failures",
                                    "single_point_failure_results_edges.csv"))[["attacked_edge_id","affected_node_id"]]

    electricity_edges_failures = pd.merge(electricity_edges_failure_results,
                                        electricity_economic_activity,
                                        how="left",
                                        left_on=["affected_node_id"],right_on=["id"]).fillna(0)
    electricity_edges_failures = electricity_edges_failures.groupby(["attacked_edge_id"])[gdp_columns].sum().reset_index()
    electricity_edges_failures["economic_loss"] = electricity_edges_failures[gdp_columns].sum(axis=1)
    electricity_edges_failures["loss_unit"] = "JD/day"
    electricity_edges_failures.to_csv(os.path.join(results_dir,
                                    "single_point_failure_electricity_edges_economic_losses_no_water.csv"),index=False)
    ...
    """
    # Read the mappings
    electricity_water_mapping = pd.read_csv(os.path.join(processed_data_path,
                                                "networks/energy",
                                                "mapping_water_to_electricity.csv"))
    electricity_water_mapping.rename(columns={"elec_node_id":"id"},inplace=True)
    # Estimate the irrigation specific dependencies
    electricity_irrigation_mapping = pd.merge(electricity_water_mapping,
                                    irrigation_economic_activity[["node_id","A_GDP"]],
                                    how="left",
                                    left_on=["water_node_id"],right_on=["node_id"]).fillna(0)
    electricity_irrigation_mapping.drop("node_id",axis=1,inplace=True)
    electricity_irrigation_mapping = electricity_irrigation_mapping.groupby(["id"])["A_GDP"].sum().reset_index()
    ...
    electricity_potable_mapping = pd.merge(electricity_water_mapping,
                                    potable_economic_activity[["node_id","W_GDP"]],
                                    how="left",
                                    left_on=["water_node_id"],right_on=["node_id"]).fillna(0)
    electricity_potable_mapping.drop("node_id",axis=1,inplace=True)
    electricity_potable_mapping = electricity_potable_mapping.groupby(["id"])["W_GDP"].sum().reset_index()
    ...
    electricity_water_buildings = pd.merge(electricity_water_mapping,
                                    potable_economic_activity_buildings,
                                    how="left",
                                    left_on=["water_node_id"],right_on=["node_id"]).fillna(0)
    electricity_water_buildings = electricity_water_buildings[electricity_water_buildings["osm_id"] > 0][["id","osm_id"]]
    del electricity_water_mapping

    electricity_economic_activity_buildings = pd.read_csv(os.path.join(processed_data_path,
                                    "networks_economic_activity",
                                    "electricity_buildings_economic_activity_mapping.csv"))[["id","osm_id"]] 
    electricity_economic_activity_buildings['osm_id'] = electricity_economic_activity_buildings['osm_id'].astype(int) 
    
    electricity_nodes_failure_results = pd.merge(electricity_nodes_failure_results,
                                        electricity_economic_activity_buildings,
                                        how="left",
                                        left_on=["affected_node_id"],
                                        right_on=["id"]).fillna(0) 
    electricity_nodes_failure_results = electricity_nodes_failure_results[["attacked_node_id","osm_id"]]
    electricity_nodes_failure_results.rename(columns={"attacked_node_id":"id"},inplace=True)
    electricity_nodes_failure_results = pd.concat([electricity_nodes_failure_results,
                                            electricity_water_buildings],
                                        axis=0,ignore_index=True)
    electricity_nodes_failure_results = electricity_nodes_failure_results.drop_duplicates(subset=["id","osm_id"],keep="first")
    electricity_nodes_failure_results = pd.merge(electricity_nodes_failure_results,buildings,how="left",on=["osm_id"]).fillna(0)
    electricity_failures = electricity_nodes_failure_results.groupby(["id"])[buildings_gdp_columns].sum().reset_index()
    print ("* Done with electricity-water-buildings mapping")

    # Merge different results together
    electricity_failures = pd.merge(electricity_failures,electricity_irrigation_mapping,how="left",on=["id"]).fillna(0)
    electricity_failures = pd.merge(electricity_failures,electricity_potable_mapping,how="left",on=["id"]).fillna(0)
    electricity_failures = pd.merge(electricity_failures,electricity_nodes_failures,how="left",on=["id"]).fillna(0)
    ...
    electricity_failures.to_csv(os.path.join(results_dir,
                                    "single_point_failure_electricity_nodes_economic_losses.csv"),index=False)
    ...
    electricity_edges_failures["loss_unit"] = "JD/day"
    electricity_edges_failures.to_csv(os.path.join(results_dir,
                                    "single_point_failure_electricity_edges_economic_losses.csv"),index=False)
