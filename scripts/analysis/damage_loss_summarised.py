        ...
        if damage_results:
            ...
            for df in damage_results:
                ...
                if asset_info.single_failure_scenarios != "none":
                    ...
                    losses.append(
                        loss[[asset_id, "economic_loss_unit"] + hazard_columns]
                    )
            ...
            if len(losses) > 0:
                ...
                if len(losses.index) > 0:
                    losses = quantiles(
                        losses, [asset_id, "economic_loss_unit"], hazard_columns
                    )
                    losses.to_parquet(
                        os.path.join(
                            summary_results_folder,
            ...
            #                     'damage_cost_unit',
            #                     ],
            #                     dropna=False).agg(sum_dict).reset_index() for df in damage_results]
