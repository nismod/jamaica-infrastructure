...
def main(config):
    ...
    for asset_info in asset_data_details.itertuples():
        ...
        if os.path.isfile(file) is True:
            ...
            nbs_risks,nbs_risk_columns = get_risks(nbs_damages.copy(),
                                            asset_id,'coastal',
                                            ['coastal'],
            ...
            risks, risk_columns = get_risks(damages.copy(),
                                            asset_id,'coastal',
                                            ['coastal'],
                                            rcps,risk_type,val_type,days=days)
            ...
            diff.reset_index().to_csv(os.path.join(nbs_benefits_results,
                                f"{asset_info.asset_gpkg}_{asset_info.asset_layer}_coastal_EAD_EAEL_change_with_mangroves.csv"),index=False)

            benefit_columns = [c.replace("risk","avoided_risk") for c in nbs_risk_columns]
            diff = risks.set_index([asset_id]).subtract(nbs_risks.set_index([asset_id]),fill_value=0)
            diff = diff.reset_index()

            diff.rename(columns=dict(zip(nbs_risk_columns,benefit_columns)),inplace=True)
            diff.to_csv(os.path.join(nbs_benefits_results,
                                f"{asset_info.asset_gpkg}_{asset_info.asset_layer}_coastal_risks_change_with_mangroves.csv"),index=False)
            ...
            gpd.GeoDataFrame(pd.merge(diff,gdf,how="left",on=[asset_id]),
                geometry="geometry",crs=f"EPSG:{epsg_jamaica}").to_file(os.path.join(nbs_benefits_results,
                                ...
                                layer=asset_info.asset_layer,driver="GPKG")
