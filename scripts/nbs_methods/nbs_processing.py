...
def intersect_layers_and_find_area(layer_i,layer_j,layer_j_id,all_ids):
    ...
    common_ij["geometry"] = common_ij.progress_apply(
                        ...
                        axis=1)
    common_ij["area_m2"] = common_ij.progress_apply(
        lambda x:x.geometry.area,
        axis=1)
    common_ij["area_hectares"] = 0.0001*common_ij["area_m2"]
    common_ij.drop(["layer_i_geometry","layer_j_geometry"],
                   axis=1,
                   inplace=True)
    common_ij = gpd.GeoDataFrame(common_ij[all_ids + ["area_m2","geometry"]],
                    geometry="geometry",
...
def main(config):
    ...
    for b in buffer_distances:
        for i in range(len(intersection_layers)-1):
            ...
            for j in intersection_layers[i+1:]:
                ...
                print (layer_j)
                common_ij = intersect_layers_and_find_area(layer_i[[f"{name_i}_id","geometry"]],
                                    layer_j[[f"{name_j}_id","geometry"]],f"{name_j}_id",
    ...
    buffer_distances = [250,500,1000]
    for b in buffer_distances:
        mangrove_layer = gpd.read_file(os.path.join(processed_data_path,
                                            "nbs",
                                            f"mangroves_buffer.gpkg"),
                                            layer=f"{b}m")
        mangrove_layer.rename(columns={"ID":"mangroves_id"},inplace=True)
        seagrass_layer = gpd.read_file(os.path.join(processed_data_path,
                                           "nbs",
                                           f"seagrass_buffer.gpkg"),
                                              layer=f"{b}m")
        seagrass_layer.rename(columns={"ID":"seagrass_id"},inplace=True)
        corals_layer = gpd.read_file(os.path.join(processed_data_path,
                                            "nbs",
                                            f"corals_buffer.gpkg"),
                                            layer=f"{b}m")
        ...
        mangroves_seagrass_corals.to_file(os.path.join(processed_data_path,
                        "nbs",
