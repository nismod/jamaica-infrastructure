...
def main(config):
    ...
    jamaica_land_area = gpd.read_file(os.path.join(processed_data_path,
                            "boundaries",
    ...
    forest_land_use = gpd.read_file(os.path.join(processed_data_path,
                            "land_type_and_use",
                            "input_land_use_layers.gpkg"),
                        layer="forest_land_use")
    forest_land_use["classes_lower"] = forest_land_use.progress_apply(lambda x:str(x["Classify"]).lower(),axis=1)
    forest_land_use = get_areas(forest_land_use)
    total_forest_land_use_area = forest_land_use["area_m2"].sum()
    print ("Total Jamaica land use area in kilometer square",1.0e-6*total_forest_land_use_area) 

    excel_writer = pd.ExcelWriter(os.path.join(processed_data_path,
                                "nbs",
                                "land_use_layers_with_areas.xlsx"))

    forest_grouping_column = "Classify"
    forest_land_use_by_class = group_areas_and_find_totals(forest_land_use,forest_grouping_column,
                                        ["Land-Use Class",
                                        "Area (m2)", 
                                        "Area (Hectares)",
    ...
    forest_only_classes = pd.read_excel(os.path.join(incoming_data_path,
                                                "nbs","NbS information.xlsx"),
                                sheet_name="forest_layers")
    forest_only_classes = [str(f).lower() for f in forest_only_classes["Forest sublayers"].values.tolist()]
    print (forest_only_classes)
    print (list(set(forest_land_use["classes_lower"].values.tolist())))
    forest_only_layers = forest_land_use[forest_land_use["classes_lower"].isin(forest_only_classes)]
    print (forest_only_layers)

    forest_only_use = group_areas_and_find_totals(forest_only_layers,forest_grouping_column,
                                        ["Land-Use Class",
                                        "Area (m2)", 
                                        "Area (Hectares)",
    ...
    agri_only_classes = pd.read_excel(os.path.join(incoming_data_path,
                                                "nbs","NbS information.xlsx"),
                                sheet_name="agriculture_forest_layers")
    agri_only_classes = [str(a).lower() for a in agri_only_classes["Agriculture sublayers"].values.tolist()]
    agri_only_layers = forest_land_use[forest_land_use["classes_lower"].isin(agri_only_classes)]
    agri_only_use = group_areas_and_find_totals(agri_only_layers,forest_grouping_column,
                                        ["Land-Use Class",
                                        "Area (m2)", 
                                        "Area (Hectares)",
    ...
    layer_name = ["mangroves",
                        "seagrass",
                         "corals"]
    areas_and_percentages = []
    for i,(input_layer,output_layer) in enumerate(list(zip(natural_layers,layer_name))):
    ...
    areas_and_percentages = pd.DataFrame(areas_and_percentages,columns=["Coastal Nbs",
                                                                        "Area (m2)", 
                                                                        "Area (Hectares)",
                                                                        "Area (% of total mainland area)"])
    areas_and_percentages = round_and_convert_to_thousands(areas_and_percentages,[("Area (m2)",0), 
                                                                        ("Area (Hectares)",0),
                                                                        ("Area (% of total mainland area)",2)])
