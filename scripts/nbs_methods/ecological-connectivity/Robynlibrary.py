...
def rasterize_condition(geodf, out_file, resolution=10):
    ...
    shapes = (
        (geom, value)
        for geom, value in zip(geodf.geometry, geodf["condition_factor_p4"])
    )  # condition_factor_p4 used to be Condition
...
def resample_and_save(
    source_raster, src_transform, src_crs, bounds, out_file, new_resolution=100
):
