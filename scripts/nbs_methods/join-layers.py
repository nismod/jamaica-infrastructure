...
def main(cell_number):
    ...
    df = geopandas.read_file(
        f"land_use_slope/extracts/jamaica_land_use_combined_with_sectors_and_slope__{cell_number-1}.gpkg"
    )

    df = df[["Classify", "LU_CODE", "cell_index", "slope_degrees", "geometry"]].rename(
        columns={"Classify": "landuse_desc", "LU_CODE": "landuse_code"}
    )
...
def slice_vector(slice_df, slice_cell):
    slice_df = slice_df.cx[
        slice_cell.left : slice_cell.right, slice_cell.bottom : slice_cell.top
    ].copy()
...
def associate_vector(fname, data_df, cell, select_cols):
    vector_df = (
        geopandas.read_file(fname)[list(select_cols.keys()) + ["geometry"]]
...
def process(with_elevation, cell):
    ...
    with_bauxite = associate_vector(
        "Terrestrial/Bauxite/nsmdb-bauxite_reserves.gpkg",
        with_soils,
        cell,
    ...
    with_primary_forest = associate_vector(
        "Terrestrial/Forests/Forests buffered 100m/Primary forest buffered 100m.shp",
        with_bauxite,
        cell,
        {"LU_CODE": "within_forest_100m"},
    )
    with_primary_forest.within_forest_100m = (
        with_primary_forest.within_forest_100m == "PF"
    )

    with_forest_reserves = associate_vector(
        "Protected Sites/Forest Reserves/Forest Reserves.shp",
        with_primary_forest,
        cell,
    ...
    with_headwater.within_headwater_stream_50m = (
        ~with_headwater.within_headwater_stream_50m.isna()
    )
