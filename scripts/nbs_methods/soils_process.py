...
def main(config):
    ...
    soil_layer["classes"] = soil_layer.progress_apply(
        lambda x: str(x["TEXTURE1"]).lower(), axis=1
    )
    print(soil_layer)
    soil_classification = pd.read_excel(
        os.path.join(incoming_data_path, "nbs", "NbS information.xlsx"),
        sheet_name="Soils",
    )
    soil_classification["classes"] = soil_classification.progress_apply(
        lambda x: str(x["Type"]).lower(), axis=1
    )
    soil_layer = pd.merge(soil_layer, soil_classification, how="left", on=["classes"])
    soil_layer["Permeability class"] = soil_layer["Permeability class"].fillna(
        "Unknown"
    )
    soil_layer["Weighting"] = soil_layer["Weighting"].fillna(0)
    soil_layer.drop("classes", axis=1, inplace=True)

    soil_layer = gpd.GeoDataFrame(
        soil_layer, geometry="geometry", crs=f"EPSG:{epsg_jamaica}"
    )
    soil_layer.to_file(
        os.path.join(processed_data_path, "nbs", "nsmdb-soils.gpkg"), driver="GPKG"
    )

    excel_writer = pd.ExcelWriter(
        os.path.join(processed_data_path, "nbs", "soil_classes_with_areas.xlsx")
    )

    soil_grouping_column = "TEXTURE1"
    soil_by_type = group_areas_and_find_totals(
        soil_layer,
        soil_grouping_column,
        [
            "Soil Type",
            "Area (m2)",
            "Area (Hectares)",
    ...
    soil_by_class = group_areas_and_find_totals(
        soil_layer,
        soil_grouping_column,
        [
            "Permeability class",
