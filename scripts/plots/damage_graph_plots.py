...
def main(config):
    ...
    plt.tight_layout()
    save_fig(os.path.join(figures_data_path, "jamaica_hazard_EAD_totals.png"))
    plt.close()
    ...
    for sector in sectors:
        hazard_eads = (
            ead_results[
                (ead_results["epoch"] == 2010) & (ead_results["sector"] == sector)
            ][ead_columns]
            .sum()
            .reset_index()
        )
    ...
    plt.tight_layout()
    save_fig(os.path.join(figures_data_path, "jamaica_hazard_sector_EAD_totals.png"))
    plt.close()

    # print (total_baseline_eaels)
    for i, (hazard, hazard_color, hazard_label) in enumerate(
        list(zip(hazards, hazard_colors, hazard_labels))
    ):
        sectors = list(set(ead_results["sector"].values.tolist()))
        hazard_df = []
        # sector_labels = []
        ...
        save_fig(
            os.path.join(
                figures_data_path, f"jamaica_hazard_sector_EAD_totals_{hazard}.png"
            )
        )
        plt.close()

    damage_results = pd.read_csv(
        os.path.join(
            damage_data_path,
    ...
    damage_columns = [
        c for c in damage_results.columns.values.tolist() if "direct_damage_cost" in c
    ]
    reopen_columns = [
        c for c in damage_results.columns.values.tolist() if "direct_reopen_cost" in c
    ]

    total_baseline_damages = (
        damage_results[
            (damage_results["epoch"] == 2010) & (damage_results["rp"] == 100)
        ][damage_columns]
        .sum()
        .reset_index()
    )
    ...
    total_baseline_damages["percentage_gdp"] = (
        100.0 * total_baseline_damages["values"] / jamaica_gdp
    )
    ...
    for hazard in hazards:
        ...
        if len(hazard_df) > 0:
            hazard_df = pd.merge(
                hazard_df, hazard_damages, how="left", on=["damage_cost"]
            )
        else:
    ...
    ax.set_ylabel(
        f"Total Damages for 1 in {rp}-yeael event (J$ Billion)",
        fontweight="bold",
        fontsize=15,
    )
    ax.set_xticklabels(hazard_labels, fontsize=15, rotation=0, fontweight="bold")
    plt.tight_layout()
    save_fig(
        os.path.join(figures_data_path, f"jamaica_hazard_1in{rp}_event_totals.png")
    )
    plt.close()

    hazard_colors = ["#08519c", "#54278f", "#016c59", "#006d2c"]
    for i, (hazard, hazard_label, hazard_color) in enumerate(
        list(zip(hazards, hazard_labels, hazard_colors))
    ):
        hazard_df = (
            damage_results[
                (damage_results["epoch"] == 2010) & (damage_results["hazard"] == hazard)
            ]
            .groupby(["hazard", "rp"])[damage_columns]
            .sum()
            .reset_index()
        )
        # print (hazards_df)
        ...
        save_fig(
            os.path.join(
                figures_data_path, f"jamaica_return_period_damages_{hazard}.png"
            )
        )
        plt.close()
    ...
    for i, (hazard, hazard_label, hazard_color) in enumerate(
        ...
        if hazard == "cyclone":
            ...
            save_fig(
                os.path.join(figures_data_path, "example_loss_probability_curve.png")
            )
            plt.close()
    ...
    for i, (hazard, hazard_color, hazard_label) in enumerate(
        ...
        if hazard in ["cyclone", "coastal"]:
            ...
            hazard_df = (
                hazard_df.groupby(["rcp", "epoch"])[ead_columns].sum().reset_index()
            )
            ...
            for j, (rcp, rcp_color) in enumerate(list(zip(rcps, rcp_colors))):
                ...
                if len(hazard_rcp.index) > 0:
                    hazard_rcp["EAD_undefended_mean"] = hazard_rcp[
                        "EAD_undefended_mean"
                    ] * (1 + currency_inflation) ** (
                        hazard_rcp["epoch"] - baseline_yeael
                    )
                    hazard_rcp = pd.concat(
                        [hazard_baseline, hazard_rcp], axis=0, ignore_index=True
                    )
                    ax.plot(
                        hazard_rcp["epoch"],
            ...
            ax.set_ylabel(
                "Average Expected Annual Damages (J$ billion)",
                fontweight="bold",
                fontsize=12,
            )
