...
def get_asset_total_loss_values(
    sector,
    asset_dataframe,
    losses_data_path,
    layer_key,
...
def main(config):
    ...
    for sector in sector_attributes:
        if sector["sector_label"] not in [
            ...
            if len(edges) > 0:
                edges_damages = get_asset_total_loss_values(
                    sector,
                    edges,
                    losses_data_path,
                    "edge",
                ...
                edges_damages.to_file(
                    os.path.join(losses_data_path, sector["sector_gpkg"]),
                    layer="edges",
                    driver="GPKG",
                )
                ...
                plot_basemap(
                    ax, processed_data_path, plot_regions=True, region_labels=True
                )
            ...
            if len(nodes) > 0:
                if sector["sector_label"] == "Ports and Airports":
                    ...
                    port_damages = get_asset_total_loss_values(
                        port_sector,
                        port_nodes,
                        losses_data_path,
                        "area",
                    ...
                    airport_damages = get_asset_total_loss_values(
                        airport_sector,
                        airport_nodes,
                        losses_data_path,
                        "area",
                    ...
                    nodes_damages = get_asset_total_loss_values(
                        sector,
                        nodes,
                        losses_data_path,
                        "node",
