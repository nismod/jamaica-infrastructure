...
def get_asset_total_damage_values(
    ...
    if len(dc) > 0:
        damage_sum_columns = [
            f"{d}_designed_protection_mean" for d in damage_sum_columns
        ]
    else:
...
def main(config):
    ...
    damage_legends = [
        "Expected Annual Damages (J$ million)",
        "Expected Annual Losses (J$ million/day)",
    ]
    damage_titles = ["Expected Annual Damages", "Expected Annual Losses"]
