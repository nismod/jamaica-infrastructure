...
def main(config):
    ...
    transport_data_path = os.path.join(processed_data_path, "networks", "transport")
    ...
    for sector in sector_attributes:
        if sector["sector_label"] in ["Roads", "Railways"]:
            ...
            save_fig(
                os.path.join(
                    figures_data_path,
                    f"{sector['sector_label'].lower().replace(' ','_')}_network.png",
                )
            )
    ...
    """
    ports = gpd.read_file(
        os.path.join(transport_data_path, "port_polygon.gpkg"), layer="areas"
    ).to_crs(epsg=JAMAICA_GRID_EPSG)
    ports["asset_type"] = "port"
    airports = gpd.read_file(
        os.path.join(transport_data_path, "airport_polygon.gpkg"), layer="areas"
    ).to_crs(epsg=JAMAICA_GRID_EPSG)
    airports["asset_type"] = "airport"
    nodes = pd.concat(
        [
            ports[["name", "asset_type", "geometry"]],
    ...
    nodes = gpd.GeoDataFrame(
        nodes, geometry="geometry", crs=f"EPSG:{JAMAICA_GRID_EPSG}"
    )
    ...
    for node_type, node_color in nodes_categories_colors.items():
        ax = plot_point_assets(
            ax,
            JAMAICA_GRID_EPSG,
            nodes[nodes["asset_type"] == node_type],
            node_color,
            15.0,
            "o",
            11,
