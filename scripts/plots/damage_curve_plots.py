...
def main(config):
    ...
    damage_data_path = os.path.join(
        incoming_data_path, "damage_assessment_information", "damage_functions"
    )

    damage_data_keys = pd.read_csv(
        os.path.join(damage_data_path, "damage_curves_keys.csv")
    )
    ...
    for data_key in damage_data_keys.itertuples():
        ...
        ax.set_title(
            f"{data_key.asset_label}: {title_label}", fontsize=14, fontweight="bold"
        )
        plt.tight_layout()
        save_fig(
            os.path.join(
