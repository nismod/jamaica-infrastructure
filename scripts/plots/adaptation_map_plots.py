...
def get_asset_adaptation_values(sector,damage_data_path,hazard_type,rcp_value,
                            damage_string,asset_dataframe,damage_sum_columns,
                            layer_key):
    ...
    if os.path.isfile(file) is True:
        ...
        return pd.merge(
                        ...
                        damages[[asset_id_colum]+damage_sum_columns],how="left",on=[asset_id_column]).fillna(0)
    else:
        return []
...
def main(config):
    ...
    damage_legends = ["NPV of adaptation costs (J$ million)",
                    "NPV of avoided risks (J$ million)",
                    "Benefit Cost Ratio"]
    damage_titles = ["Adaptation investments Net Present Values",
    ...
    for hazard in ["flooding","TC"]:
        v_cols = ["adapt_cost_npv"] + [f"{hazard}__rcp_{rcps}__{c}" for c in damage_types[1:]]
        ...
        for sector in sector_attributes:
            ...
            if len(edges) > 0:
                edges_damages = get_asset_adaptation_values(sector,
                                                            damage_data_path,
                                                            hazard_type,rcps,
                                                            damage_string,
                                                            edges,
            ...
            if len(nodes) > 0:
                if sector["sector_label"] == "Ports and Airports":
                    ...
                    port_damages = get_asset_adaptation_values(port_sector,
                                                            damage_data_path,
                                                            hazard_type,rcps,
                                                            damage_string,
                                                            port_nodes,
                    ...
                    airport_damages = get_asset_adaptation_values(airport_sector,
                                                            damage_data_path,
                                                            hazard_type,rcps,
                                                            damage_string,
                                                            airport_nodes,
                    ...
                    nodes_damages = get_asset_adaptation_values(sector,
                                                            damage_data_path,
                                                            hazard_type,rcps,
                                                            damage_string,
                                                            nodes,
