# shapefiles
island = gpd.read_file('/soge-home/projects/mistral/jamaica-ccri/incoming_data/water/potable/jamaica-latest-free.shp/jam_admbnda_adm0_regrid.shp', crs="EPSG:3448")
pipeline = gpd.read_file('/soge-home/projects/mistral/jamaica-ccri/incoming_data/water/potable/raw/pipelines_network_NWC.shp', crs="EPSG:3448")
river_network = gpd.read_file('/soge-home/projects/mistral/jamaica-ccri/incoming_data/water/natural/river_network.shp', crs="EPSG:3448")
major_rivers = gpd.read_file('/soge-home/projects/mistral/jamaica-ccri/incoming_data/water/natural/major_rivers.shp', crs="EPSG:3448")
hydrobasins = gpd.read_file('/soge-home/projects/mistral/jamaica-ccri/incoming_data/water/natural/hydrobasins.shp', crs="EPSG:3448")

licensed_sources = gpd.read_file('/soge-home/projects/mistral/jamaica-ccri/drought/economic/licensed_abstractions.shp', crs="EPSG:3448")
...
licensed_sources = licensed_sources[licensed_sources['Expiration'].isin(['Not Expired','#VALUE!',''])]
licensed_sources['Source'] = np.where(licensed_sources['Source_Typ']=='Well','GW','SW')
licensed_sources['Purpose_Us'] = licensed_sources['Purpose_Us'].astype(str)

licensed_sources['VBF_Class'] = np.where(licensed_sources['Purpose_Us'].str.contains('Public'),'Potable',
                                    np.where(licensed_sources['Purpose_Us'].str.contains('Potable'),'Potable',
                                    np.where(licensed_sources['Purpose_Us'].str.contains('Irrigation'),'Irrigation',
                                    np.where(licensed_sources['VBF_Class'].str.contains('Agricultural'),'Agriculture',
                                    np.where(licensed_sources['Purpose_Us'].str.contains('Indus'),'Industrial',licensed_sources['VBF_Class'])))))
...
dict = {'Agriculture':'green', 'Domestic':'blue', 'Hydropower':'navy', 'Industrial':'darkgoldenrod', 'Irrigation':'lime', 'Potable':'cyan', 'Recreation':'purple'}
for name, group in groups:
    c = dict[name]
    ax.scatter(group.lon, group.lat, marker='o', c=c, label=name+'SW', s=group.VolGrantm3/250, edgecolor='k') #, edgecolor='k'

groups = licensed_sources_gw.groupby('VBF_Class')
dict = {'Agriculture':'green', 'Domestic':'blue', 'Hydropower':'navy', 'Industrial':'darkgoldenrod', 'Irrigation':'lime', 'Potable':'cyan', 'Recreation':'purple'}
for name, group in groups:
    c = dict[name]
    ax.scatter(group.lon, group.lat, marker='d', c=c, label=name+'GW', s=group.VolGrantm3/250, edgecolor='k') #

msizes = [round(licensed_sources['VolGrantm3'].quantile(0.25)/250,0), 
...
markers = []
for size in msizes:
   markers.append(ax.scatter([],[], c='k', s=size, label=str(size*500)+' Annual volume of abstraction (m3)'))
lgnd = ax.legend()
