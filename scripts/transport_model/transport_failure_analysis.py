...
def main(config,min_node_number,max_node_number):
    ...
    losses = edge_fail_results.groupby(["edge_id","no_access"])["time_loss",
                                                            "labour_rerouting_loss",
                                                            "trade_rerouting_loss",
                                                            "labour_gdp_loss","trade_loss"].sum().reset_index()
    rerouting_times_min = edge_fail_results.groupby(["edge_id","no_access"])["time_loss"].min().reset_index()
    rerouting_times_min.rename(columns={"time_loss":"min_trip_time_loss"},inplace=True)
    rerouting_times_max = edge_fail_results.groupby(["edge_id","no_access"])["time_loss"].max().reset_index()
    rerouting_times_max.rename(columns={"time_loss":"max_trip_time_loss"},inplace=True)
    rerouting_times_mean = edge_fail_results.groupby(["edge_id","no_access"])["time_loss"].mean().reset_index()
