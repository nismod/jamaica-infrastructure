...
def main(config):
    ...
    airports = gpd.read_file(os.path.join(processed_data_path,
                                    'networks',
                                    'transport',
                                    'airport_polygon.gpkg'),
                            layer='areas')
    ...
    rail_nodes = rail_nodes[(rail_nodes["asset_type"] == "station") & (rail_nodes["status"] == "Functional")]
    ...
    """
    multi_modal = []
    multi_modal.append(map_nearest_locations_and_create_lines(airports.copy(),road_nodes.copy(),"node_id","node_id","air","road"))
    multi_modal.append(map_nearest_locations_and_create_lines(ports.copy(),road_nodes.copy(),"node_id","node_id","port","road"))
    multi_modal.append(map_nearest_locations_and_create_lines(ports.copy(),rail_nodes.copy(),"node_id","node_id","port","rail"))
    multi_modal.append(map_nearest_locations_and_create_lines(rail_nodes.copy(),road_nodes.copy(),"node_id","node_id","rail","road"))

    """Add road and rail
    """

    multi_modal = gpd.GeoDataFrame(pd.concat(multi_modal,axis=0,ignore_index=True),
                        geometry="geometry",crs=f"EPSG:{epsg_jamaica}")
    multi_modal["edge_id"] = multi_modal.index.values.tolist()
    multi_modal["edge_id"] = multi_modal.progress_apply(lambda x: f"multie_{x.edge_id}",axis=1)
    ...
    road_edges = gpd.read_file(os.path.join(processed_data_path,
                                    ...
                                    'roads.gpkg'),
                            layer='edges')
    road_edges_max_id = max([int(c.split("_")[1]) for c in road_edges["edge_id"].values.tolist()])
    ...
    new_road["speed"] = [110]
    new_road["geometry"] = [LineString([road_nodes[road_nodes["node_id"] == "roadsn_84980"].geometry.values[0],
    ...
    multi_modal = gpd.GeoDataFrame(pd.concat([multi_modal[columns],rail_edges[columns],road_edges[columns],new_road[columns]],
                            axis=0,ignore_index=True),
                            geometry="geometry",crs=f"EPSG:{epsg_jamaica}")
    print (multi_modal)
    multi_modal.to_file(os.path.join(processed_data_path,
                                    'networks',
                                    'transport',
                                    'multi_modal_network.gpkg'),
                            layer='edges',driver="GPKG")
    columns = ["node_id","mode","geometry"]
    multi_modal = gpd.GeoDataFrame(pd.concat([airports[columns],ports[columns],
                            rail_nodes[columns],road_nodes[columns]],
                            axis=0,ignore_index=True),
                            geometry="geometry",crs=f"EPSG:{epsg_jamaica}")
    print (multi_modal)
    multi_modal.to_file(os.path.join(processed_data_path,
