...
def route_mines_to_nearest_ports(areas,nodes,edges,ports,connection_type="mines"):
    ...
    network = pd.concat([edges[columns],
                        nearest_roads[columns],
                        nearest_stations[columns]],
    ...
    od_pairs = [list(zip([b]*len(all_ports),all_ports)) for b in areas.mining_id.values.tolist()]
    od_pairs = [item for sublist in od_pairs for item in sublist]
    od_pairs = pd.DataFrame(od_pairs,columns=["origin_id","destination_id"])
    od_pairs = pd.merge(od_pairs,areas[["mining_id","area_m2"]],how="left",left_on=["origin_id"],right_on=['mining_id'])
    flow_paths = network_od_paths_assembly(od_pairs[["origin_id","destination_id","area_m2"]],
                                G,
                                "time","area_m2")
    flow_paths = flow_paths.sort_values(by="gcost")
    flow_paths = flow_paths.drop_duplicates(subset=["origin_id"],keep="first")

    return flow_paths, pd.concat([
                        nearest_roads[columns],
                        nearest_stations[columns]],
                        axis=0,ignore_index=True)[columns]
...
def main(config):
    ...
    print (quarry_areas)
    bauxite_areas = mining_areas[~mining_areas["mining_id"].isin(quarry_areas["mining_id"].values.tolist())]
    print (bauxite_areas) 

    quarry_areas["geometry"] = quarry_areas.progress_apply(lambda x:x.geometry.centroid,axis=1)
    quarry_areas = quarry_areas.to_crs(epsg=epsg_jamaica)
    print (quarry_areas)

    bauxite_areas["geometry"] = bauxite_areas.progress_apply(lambda x:x.geometry.centroid,axis=1)
    ...
    """
    ports = gpd.read_file(os.path.join(processed_data_path,
                                    'networks',
                                    'transport',
                                    'port_polygon.gpkg'),
                            layer='areas')
    ...
    quarry_to_ports, quarry_network = route_mines_to_nearest_ports(quarry_areas,nodes,edges,ports,connection_type="quarry")
    bauxite_to_ports, bauxite_network = route_mines_to_nearest_ports(bauxite_areas,nodes,edges,ports)

    quarry_to_ports_areas = quarry_to_ports.groupby(["destination_id"])["area_m2"].sum().reset_index()
    quarry_to_ports_areas.rename(columns={"area_m2":"tot_area"},inplace=True)
    bauxite_to_ports_areas = bauxite_to_ports.groupby(["destination_id"])["area_m2"].sum().reset_index()
    bauxite_to_ports_areas.rename(columns={"area_m2":"tot_area"},inplace=True)
    quarry_to_ports = pd.merge(quarry_to_ports,quarry_to_ports_areas,how="left",on=["destination_id"])
    bauxite_to_ports = pd.merge(bauxite_to_ports,bauxite_to_ports_areas,how="left",on=["destination_id"])
    ...
    quarry_output = (1+tax_rate)*(1.0e6/365.0)*economic_output[(
                                        economic_output["sector_code"] == "C"
                                        ) & (
                                        economic_output["subsector_code"] == "141"
                                        )][f"{financial_year}"].sum()

    bauxite_output = (1+tax_rate)*(1.0e6/365.0)*economic_output[(
                                        economic_output["sector_code"] == "C"
                                        ) & (
                                        economic_output["subsector_code"] == "132"
                                        )][f"{financial_year}"].sum()
    print ("* Given GDP",quarry_output + bauxite_output)
    quarry_to_ports = pd.merge(quarry_to_ports,
                            ports[["node_id","export_wt"]],
                            how="left",left_on=["destination_id"],right_on=["node_id"])
    quarry_to_ports["C_GDP"] = quarry_output*quarry_to_ports["export_wt"]*quarry_to_ports["area_m2"]/quarry_to_ports["tot_area"]
    bauxite_to_ports = pd.merge(bauxite_to_ports,
                            ports[["node_id","export_wt"]],
                            how="left",left_on=["destination_id"],right_on=["node_id"])
    bauxite_to_ports["C_GDP"] = bauxite_output*bauxite_to_ports["export_wt"]*bauxite_to_ports["area_m2"]/bauxite_to_ports["tot_area"] 
    quarry_to_ports["GDP_persqm"] = quarry_to_ports["C_GDP"]/quarry_to_ports["area_m2"]
    bauxite_to_ports["GDP_persqm"] = bauxite_to_ports["C_GDP"]/bauxite_to_ports["area_m2"]

    mining_gdp = pd.concat([quarry_to_ports[["origin_id","C_GDP","GDP_persqm"]],
    ...
    mining_areas = gpd.GeoDataFrame(mining_areas,
                    geometry="geometry",
    ...
    if write_results is True:
        mining_areas.to_file(os.path.join(processed_data_path,
                                    "mining_data",
                                    "mining_gdp.gpkg"),
                                layer="areas",driver="GPKG")
    flow_paths = pd.concat([quarry_to_ports,
                            bauxite_to_ports],
                            axis=0,ignore_index=True)
    flow_paths[["origin_id","destination_id",
            "edge_path","gcost","C_GDP"]].to_csv(os.path.join(results_path,
                                    'flow_mapping',
                                    'mines_flow_paths.csv'),
                            index=False)
    edge_flows = get_flow_on_edges(flow_paths,"edge_id","edge_path",
                                                    "C_GDP")
    columns = ["from_node","to_node","edge_id","from_mode","to_mode","length_m","speed","time","geometry"]
    network = pd.concat([edges[columns],
                        quarry_network[columns],
                        bauxite_network[columns]],
                        axis=0,ignore_index=True)[columns]
    edge_flows = gpd.GeoDataFrame(pd.merge(edge_flows,
                    network,how="left",on=["edge_id"]),
                    geometry="geometry",
                    crs=f"EPSG:{epsg_jamaica}")
