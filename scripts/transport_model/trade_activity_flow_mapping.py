...
def main(config):
    ...
    """
    jam_ports = gpd.read_file(os.path.join(processed_data_path,
                                    'networks',
                                    'transport',
                                    'port_polygon.gpkg'),
                            layer='areas')
    ...
    mining_ports = port_import_exports(jam_ports[jam_ports["commodity"] == "alumina"],"export_tonnes","mining")
    non_mining_export_ports = port_import_exports(
                    jam_ports[(jam_ports["commodity"] != "alumina") & (jam_ports["export_tonnes"] > 0)],
                    "export_tonnes","export")
    non_mining_import_ports = port_import_exports(
                    jam_ports[(jam_ports["commodity"] != "alumina") & (jam_ports["import_tonnes"] > 0)],
                    "import_tonnes","import")
    petrojam_port = port_import_exports(
                    jam_ports[jam_ports["name"] == "Petrojam"],
                    "import_tonnes","petrojam")

    all_ports = pd.concat([mining_ports,
                        non_mining_export_ports,
                        non_mining_import_ports,
                        petrojam_port],
    ...
    """
    financial_year = 2019
    exports = pd.read_excel(os.path.join(processed_data_path,
                            "macroeconomic_data",
                            "domestic_export_by_sector.xlsx"),sheet_name=str(financial_year))
    exports.columns = [str(c).strip() for c in exports.columns.values.tolist()]
    exports = exports[~exports["subsector_code"].isin(["ALL","SUB"])]
    exports = exports.groupby(["sector_code","sector_type"])[str(financial_year)].sum().reset_index()
    exports = exports[exports["sector_code"].isin(["A","B","C","D"])]   # These are mostly the sectors exporting commodities
    ...
    """
    imports = pd.read_excel(os.path.join(processed_data_path,
                            "macroeconomic_data",
                            "import_by_industry.xlsx"),sheet_name=str(financial_year))
    fuel_shares = pd.read_excel(os.path.join(processed_data_path,
                            "macroeconomic_data",
                            "import_by_industry.xlsx"),sheet_name="fuel_shares")
    imports.columns = [str(c).strip() for c in imports.columns.values.tolist()]
    imports = imports[~imports["subsector_code"].isin(["ALL","SUB"])]
    imports_groups = imports.groupby(["sector_code","sector_type"])[f"{financial_year}"].sum().reset_index()
    imports_fuels = imports_groups[imports_groups["sector_code"] == "C,D,G"][f"{financial_year}"].sum()
    ...
    imports = pd.concat([imports_groups,
                        fuel_shares[["sector_code","sector_type",str(financial_year)]]],
                        axis=0,ignore_index=True)
    imports = imports.groupby(["sector_code","sector_type"])[f"{financial_year}"].sum().reset_index()
    ...
    for trade_details in trade.itertuples():
        ...
        if trade_details.sector_type == "Agriculture":
            gdp_areas = gpd.read_file(os.path.join(processed_data_path,
                                "agriculture_data",
                                "agricuture_gdp.gpkg"),
                            layer="areas")
            ...
            mining_areas["mining_id"] = mining_areas.progress_apply(lambda x:f"mines_{x.mining_id}",axis=1)
            ...
            if trade_details.sector_type == "Quarrying":
                ...
                gdp_areas = mining_areas[~mining_areas["mining_id"].isin(quarry_areas["mining_id"].values.tolist())].copy()
        ...
        gdp_areas["geometry"] = gdp_areas.progress_apply(lambda x:x.geometry.centroid,axis=1)
        gdp_areas = gdp_areas.to_crs(epsg=epsg_jamaica)
        
        print (gdp_areas)
        print (all_ports[all_ports[port_wt] > 0])

        sector_to_ports, sector_edges = route_areas_to_nearest_ports(gdp_areas,id_column,
                                            gdp_value,nodes.copy(),edges.copy(),all_ports[all_ports[port_wt] > 0],port_wt,
                                            connection_type=trade_details.sector_code,
                                            trade_type=trade_details.trade_type,
                                            include_rail=include_rail)
        sector_to_ports[f"{trade_details.sector_code}_trade"] = trade_details.trade_value*sector_to_ports["trade_wt"]
        ...
        #                         index=False)
        edge_flows = get_flow_on_edges(sector_to_ports,"edge_id","edge_path",
    ...
    pd.concat(sector_flows,axis=0,ignore_index=True).to_csv(os.path.join(results_path,
                                        'flow_mapping',
                                        f'sector_to_ports_flow_paths.csv'),
                                index=False)
    sector_network = pd.concat(sector_network,
                        axis=0,ignore_index=True)[columns]
    flow_network = pd.concat(flow_network,
                        axis=0,ignore_index=True)
    flow_gdp_columns= [c for c in flow_network.columns.values.tolist() if "_trade" in c]
    for f in flow_gdp_columns:
        flow_network[f] = flow_network[f].fillna(0)
    flow_index_columns = [c for c in flow_network.columns.values.tolist() if c not in flow_gdp_columns]
