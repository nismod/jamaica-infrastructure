...
def route_areas_to_nearest_ports(areas,areas_id,areas_gdp,
                            nodes,edges,ports,port_weight,
                            connection_type="areas",trade_type="import",include_rail=False):
    network_columns = ["from_node","to_node","edge_id",
                        "from_mode","to_mode","length_m",
                        "speed","time","geometry"]
    nearest_roads = map_nearest_locations_and_create_lines(areas.copy(),
                                    nodes[nodes["mode"] == "road"].copy(),
                                    areas_id,"node_id",connection_type,"road")
    nearest_roads["edge_id"] = nearest_roads.progress_apply(lambda x:f"{connection_type}roade_{x.name}",axis=1)
    nearest_roads["speed"] = 10.0
    nearest_roads["time"] = 0.001*nearest_roads["length_m"]/nearest_roads["speed"]

    if include_rail is True:
        nearest_stations = map_nearest_locations_and_create_lines(areas.copy(),
                                    nodes[nodes["mode"] == "rail"].copy(),
                                    areas_id,"node_id",connection_type,"rail")
        ...
        network = pd.concat([edges[network_columns],
                            ...
                            axis=0,ignore_index=True)[network_columns]
    ...
    od_pairs = [list(zip(all_ports,[b]*len(all_ports))) for b in areas[areas_id].values.tolist()]
    od_pairs = [item for sublist in od_pairs for item in sublist]
    od_pairs = pd.DataFrame(od_pairs,columns=["origin_id","destination_id"])
    od_pairs = pd.merge(od_pairs,areas[[areas_id,areas_gdp]],how="left",left_on=["destination_id"],right_on=[areas_id])
    flow_paths = network_od_paths_assembly(od_pairs[["origin_id","destination_id",areas_gdp]],
                                G,
                                "time",areas_gdp)
    flow_paths = flow_paths.sort_values(by="gcost")
    flow_paths = flow_paths.drop_duplicates(subset=["destination_id"],keep="first")
    flow_paths = flow_paths[["origin_id","destination_id","edge_path","gcost",areas_gdp]]
    ...
    flow_paths = pd.merge(flow_paths,
                        ports[["node_id",port_weight]],
                        how="left",left_on=["origin_id"],right_on=["node_id"])
    flow_paths["trade_wt"] = flow_paths[port_weight]*flow_paths[areas_gdp]/flow_paths["tot_GDP"]
    flow_paths = flow_paths[["origin_id","destination_id","edge_path","gcost","trade_wt"]]

    if trade_type == "export":
        flow_paths.columns = ["destination_id","origin_id","edge_path","gcost","trade_wt"]
...
def filter_sector_from_buildings(buildings_dataframe,sector_code,subsector_code):
    ...
    get_sector["find_subsector"] = get_sector.progress_apply(lambda x: 1 if subsector_code in str(x.subsector_code) else 0,axis=1)
...
def main(config):
    ...
    """
    columns = ["from_node","to_node","edge_id","from_mode","to_mode","length_m","speed","time","geometry"]
    trade_flow_edges = gpd.read_file(os.path.join(results_path,
                                    'flow_mapping',
                                    f'sector_imports_exports_to_ports_flows.gpkg'),
                            layer='edges')
    # print (trade_flow_edges)
    trade_sectors = [s for s in list(set(trade_flow_edges["from_mode"].values.tolist())) if s not in ["road","rail","port","air"]]
    # print (trade_sectors)
    labour_flow_edges = gpd.read_file(os.path.join(processed_data_path,
                                    'networks',
                                    'transport',
                                    'multi_modal_network.gpkg'),
                            layer='edges')
    labour_flow_edges = labour_flow_edges[(labour_flow_edges["from_mode"] == "road") & (labour_flow_edges["to_mode"] == "road")][columns]
    # print (labour_flow_edges)
    # G = ig.Graph.TupleList(network.itertuples(index=False), edge_attrs=list(network.columns)[2:])
    
    trade_flows = pd.read_csv(os.path.join(results_path,
                                'flow_mapping',
                                'sector_to_ports_flow_paths.csv'))
    trade_flows["edge_path"] = trade_flows.progress_apply(lambda x:ast.literal_eval(x.edge_path),axis=1)
    # print (trade_flows)

    labour_flows = pd.read_csv(os.path.join(results_path,
                                        'flow_mapping',
                                        'labour_to_sectors_trips_and_activity_reduced_sample.csv'))
    labour_flows["edge_path"] = labour_flows.progress_apply(lambda x:ast.literal_eval(x.edge_path),axis=1)
    # print (labour_flows)

    all_flows = pd.concat([trade_flows,labour_flows],axis=0,ignore_index=True)
    all_flows = all_flows[["origin_id","destination_id",
                        "gcost","working_trips",
                        "GDP_to_trips"] + [f"{t}_trade" for t in trade_sectors]]
    labour_flow_path_indexes = tf.get_flow_paths_indexes_of_edges(labour_flows,"edge_path")
    network_dictionary = [{"network":labour_flow_edges,"flows":labour_flows,"edge_indexes":labour_flow_path_indexes}]
    ...
    for t in trade_sectors:
        ...
        sector_flows = trade_flows[trade_flows[f"{t}_trade"] > 0][["origin_id",
                                                        "destination_id","edge_path",
                                                        "gcost",f"{t}_trade"]].reset_index()
        sector_flow_path_indexes = tf.get_flow_paths_indexes_of_edges(sector_flows,"edge_path")
        network_dictionary.append({"network":sector_network,"flows":sector_flows,"edge_indexes":sector_flow_path_indexes})
    ...
    for node_number in range(0,len(rail_nodes)):
        ...
        edge_fail = edges[(edges["from_node"] == node_fail) | (edges["to_node"] == node_fail)] 
        if len(edge_fail.index) > 0:
            ...
            for networks in network_dictionary: 
                edge_fail_results += tf.igraph_scenario_edge_failures(networks["network"], 
                                    node_edges,
                                    networks["flows"],networks["edge_indexes"], 
                                    "edge_path",
                                    "time") 
    ...
    losses = edge_fail_results.groupby(["node_id","no_access"])["time_loss",
                                                            "labour_rerouting_loss",
                                                            "trade_rerouting_loss",
                                                            "labour_gdp_loss","trade_loss"].sum().reset_index()
    rerouting_times_min = edge_fail_results.groupby(["node_id","no_access"])["time_loss"].min().reset_index()
    rerouting_times_min.rename(columns={"time_loss":"min_trip_time_loss"},inplace=True)
    rerouting_times_max = edge_fail_results.groupby(["node_id","no_access"])["time_loss"].max().reset_index()
    rerouting_times_max.rename(columns={"time_loss":"max_trip_time_loss"},inplace=True)
    rerouting_times_mean = edge_fail_results.groupby(["node_id","no_access"])["time_loss"].mean().reset_index()
    rerouting_times_mean.rename(columns={"time_loss":"mean_trip_time_loss"},inplace=True)
    
    losses = pd.merge(losses,rerouting_times_min,how="left",on=["node_id","no_access"])
    losses = pd.merge(losses,rerouting_times_max,how="left",on=["node_id","no_access"])
    losses = pd.merge(losses,rerouting_times_mean,how="left",on=["node_id","no_access"])

    losses.to_csv(os.path.join(results_path,
                            "transport_failures",
                            f"single_station_failures_scenarios.csv"),
                    index=False)
