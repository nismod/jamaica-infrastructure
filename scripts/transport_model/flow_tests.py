...
def route_areas_to_nearest_ports(areas,areas_id,areas_gdp,nodes,edges,ports,connection_type="areas",flow_type="exports"):
    ...
    roads_columns = ["from_node","to_node","edge_id","from_mode","to_mode","length_m","speed","time","geometry"]
    network = pd.concat([edges[roads_columns],
                        nearest_roads[roads_columns]],
                        axis=0,ignore_index=True)[roads_columns]
    ...
    if flow_type == "exports":
        flow_paths.columns = ["destination_id","origin_id","edge_path","gcost",areas_gdp]
