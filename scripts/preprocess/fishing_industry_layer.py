...
def main(config):
    ...
    fishing_locations = gpd.read_file(os.path.join(incoming_data_path,
                                        "nsdmb", 
    ...
    polyfit = np.polyfit(prod_locations["Shape_Area"],prod_locations["Size_Farm"],deg=1)
    noprod_locations = fishing_locations[fishing_locations["Size_Farm"] == 0]
    noprod_locations["Size_Farm"] = polyfit[0]*noprod_locations["Shape_Area"] + polyfit[1]
    
    fishing_locations = gpd.GeoDataFrame(
                                pd.concat([prod_locations,noprod_locations],
                                    axis=0,
                            ignore_index=True),
                            geometry="geometry",crs=f"EPSG:{epsg_jamaica}")
    fishing_locations["farm_id"] = fishing_locations.index.values.tolist()
    fishing_locations.to_file(os.path.join(processed_data_path,
