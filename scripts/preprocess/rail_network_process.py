...
def get_node_status(x, edges):
    edge_status = list(
        set(
            edges[(edges.from_node == x.node_id) | (edges.to_node == x.node_id)][
                "status"
            ].values.tolist()
        )
    )
...
def main(config):
    ...
    print(nodes)
    network = create_network_from_nodes_and_edges(nodes, edges, "rail")
    print(network.edges)
    print(network.nodes)
    edges = network.edges
    nodes = network.nodes
    ...
    min_cost = (
        0.001 * 1.0e6 / 1.609 / 0.0067
    )  # Cost estimate obtained from Ministry is 1,000,000 US$/mile
    max_cost = (
        0.001 * 1.2 * 1.0e6 / 1.609 / 0.0067
    )  # Cost estimate obtained from Ministry is 1,200,000 US$/mile
    ...
    nodes["asset_type"] = nodes.progress_apply(
        lambda x: "station" if str(x.Station) not in ("None", "nan") else "junction",
        axis=1,
    )
    ...
    component_file = os.path.join(
        processed_data_path, "networks", "transport", "rail.gpkg"
    )
    edges_components = assign_network_components.main(
        component_file, component_file, "node_id"
    )
    ...
    for i in range(len(components) - 1):
        node_component = nodes[nodes["component"] == components[i]][
            ["node_id", "geometry"]
        ]
        remaining_components = nodes[nodes["component"].isin(components[i + 1 :])][
            ["node_id", "geometry"]
        ]
        component_distances = map_nearest_locations_and_create_lines(
            node_component, remaining_components, "node_id", "node_id", "rail", "rail"
        )
        joined_components.append(
            component_distances[component_distances["length_m"] <= 150]
        )
    ...
    joined_components["edge_id"] = joined_components.progress_apply(
        lambda x: f"raile_{x.edge_id}", axis=1
    )
    ...
    for row in joined_components.itertuples():
        ...
        joined_components.loc[joined_components["edge_id"] == row.edge_id, "status"] = (
            values[0]
        )
    ...
    edges = gpd.GeoDataFrame(
        pd.concat([edges, joined_components], axis=0, ignore_index=True),
        geometry="geometry",
        crs=f"EPSG:{epsg_jamaica}",
    )
    edges["asset_type"] = edges.progress_apply(
        lambda x: "rail" if x.status == "Functional" else x.status, axis=1
    )
