...
def main(config):
    ...
    for layers in layer_details:
        ...
        out_fname = os.path.join(output_path,
                                f"{layers['layer_type']}.gpkg")
        for d in range(len(layers['nsdmb_names'])):
            nsdmb_layer = gpd.read_file(os.path.join(
                                    database_path,
                                    'nsdmb',
                                    database_name),
                        layer=layers['nsdmb_names'][d])
            nsdmb_layer.to_file(out_fname, 
                    layer=layers['gpkg_names'][d], 
                    driver='GPKG')
