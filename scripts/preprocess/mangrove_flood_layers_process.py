...
def main(config):
    ...
    mangrove_input_path = os.path.join(
        incoming_data_path, "forces_of_nature_project_data", "Flood Extent Rasters"
    )
    ...
    for rp in return_periods:
        for mg in mangrove_cases:
            ...
            fpath_out = os.path.join(
                mangrove_output_path, f"{flood_file}_reproject.tif"
            )
            raster_rewrite(fpath_in, fpath_out)

            outCSVName = os.path.join(
                mangrove_output_path, f"{flood_file}_reproject.csv"
            )
            ...
            load_points = pd.read_csv(
                outCSVName, header=None, names=["x", "y", flood_file], index_col=None
            )
            os.remove(outCSVName)
            os.remove(fpath_out)
            geometry = [Point(xy) for xy in zip(load_points.x, load_points.y)]
            load_points = gpd.GeoDataFrame(
                load_points, crs=f"epsg:{epsg_jamaica}", geometry=geometry
            )
            ...
            if len(flood_points) > 0:
                ...
                matches = gpd.sjoin(
                    flood_points, load_points, how="inner", op="intersects"
                ).reset_index()
                flood_points = pd.merge(
                    flood_points,
                    matches[["x", "y", flood_file]],
                    how="left",
                    on=["x", "y"],
                )
            else:
    ...
    flood_points.to_csv(
        os.path.join(mangrove_output_path, "forces_to_nature_flood_results_raw.csv"),
        index=False,
    )

    flood_points = pd.read_csv(
        os.path.join(mangrove_output_path, "forces_to_nature_flood_results_raw.csv")
    )
    flood_points["totals"] = 0
    for rp in return_periods:
        for mg in mangrove_cases:
            flood_points[f"RP_{rp}_{mg}"] = np.where(
                flood_points[f"RP_{rp}_{mg}"] < 0, 0, flood_points[f"RP_{rp}_{mg}"]
            )
            flood_points["totals"] += flood_points[f"RP_{rp}_{mg}"]

    floods_filtered = flood_points[flood_points["totals"] > 0]
    flood_points.drop("totals", axis=1).to_csv(
        os.path.join(mangrove_output_path, "forces_to_nature_flood_results_masked.csv"),
        index=False,
    )
    floods_filtered.drop("totals", axis=1).to_csv(
        os.path.join(
            mangrove_output_path, "forces_to_nature_flood_results_filtered.csv"
        ),
        index=False,
    )
