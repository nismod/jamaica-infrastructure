...
def main(config):
    ...
    electricity_nodes = gpd.read_file(
        os.path.join(
            processed_data_path, "networks", "energy", "electricity_network_v0.1.gpkg"
        ),
        layer="nodes_processed",
    )
    electricity_nodes = electricity_nodes[electricity_nodes["asset_type"] == "sink"]
    electricity_id_column = "id"

    parish_boundaries = gpd.read_file(
        os.path.join(processed_data_path, "boundaries", "admin_boundaries.gpkg"),
        layer="admin1",
    )

    population_boundaries = gpd.read_file(
        os.path.join(processed_data_path, "boundaries", "admin_boundaries.gpkg"),
        layer="admin3",
    )
    ...
    """
    electricity_nodes = assign_node_weights_by_population_proximity(
        electricity_nodes,
