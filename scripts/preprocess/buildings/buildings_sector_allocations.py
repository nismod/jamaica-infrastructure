...
def sector_subsector_asset_mapping(
    building_dataframe, building_sector_mapping_file, columns_types
):
    merge_columns = []
    new_columns = []
    for column in columns_types:
        sector_map = pd.read_excel(
            os.path.join(
                building_dataframe_data_path, "hotosm_mapping_economic_sectors.xlsx"
            ),
            building_sector_mapping_file,
            sheet_name=column,
        )
        sector_map = sector_map.drop_duplicates(
            [column, "sector_code", "subsector_code"], keep="first"
        )
    ...
    building_dataframe["shop"] = building_dataframe["shop"].replace(
        "yes", "wholesale", regex=True
    )
    building_dataframe["assigned_attribute"] = building_dataframe.progress_apply(
        lambda x: ",".join(
            [str(x[c]) for c in columns_types if str(x[c]) not in ["None", "none"]]
        ),
        axis=1,
    )
...
def match_points_to_nearest_buildings(buildings_input, points_file, sector_codes):
    ...
    buildings_select = buildings_input[
        buildings_input["sector_code"].isin(sector_codes)
    ].reset_index()
...
def main(config):
    ...
    output_excel = os.path.join(
        incoming_data_path, "buildings", "unique_column_mapping.xlsx"
    )
    ...
    buildings_input[new_columns] = buildings_input[new_columns].replace(
        [r"^\s+$", "nan"], "X", regex=True
    )

    buildings_input["sector_subsector"] = buildings_input.progress_apply(
        lambda x: get_sector_subsector(x, merge_columns), axis=1
    )
    buildings_input[["sector_code", "subsector_code"]] = buildings_input[
        "sector_subsector"
    ].apply(pd.Series)
    ...
    buildings_input["centroid"] = buildings_input.progress_apply(
    ...
    points_layers_data = os.path.join(
        processed_data_path, "locations", "building_dataframe.gpkg"
    )
    ...
    for layer in nsdmb_layers.itertuples():
        ...
        points_file.to_csv(
            os.path.join(building_data_path, "test_values.csv"), index=False
        )
        building_ids = list(
            set(
                points_file[points_file["distance"] < distance_buffer][
                    "bid"
                ].values.tolist()
            )
        )
        ...
        # print (building_ids)
        buildings_input.loc[
            buildings_input["bid"].isin(building_ids), "sector_code"
        ] = sector_code
        buildings_input.loc[
            buildings_input["bid"].isin(building_ids), "subsector_code"
        ] = subsector_code
        buildings_input.loc[
            buildings_input["bid"].isin(building_ids), "assigned_point"
        ] = layer_name
    ...
    for layer in layer_dict:
        ...
        if layer["layer_name"] == "industry":
            ...
            for sh in range(len(layer["excel_sheets"])):
                nsdmb_layers = pd.read_excel(
                    os.path.join(building_data_path, layer["excel_file"]),
                    sheet_name=layer["excel_sheets"][sh],
                )
                points_file = pd.merge(
                    points_file, nsdmb_layers, how="left", on=layer["layer_columns"][sh]
                )
                points_file.rename(
                    columns={
                        "sector_code": f"{layer['excel_sheets'][sh]}_sector_code",
                        "subsector_code": f"{layer['excel_sheets'][sh]}_subsector_code",
                    },
                    inplace=True,
                )

                points_file[f"{layer['excel_sheets'][sh]}_sector_code"].fillna(
                    "", inplace=True
                )
                points_file[f"{layer['excel_sheets'][sh]}_subsector_code"].fillna(
                    "", inplace=True
                )

            points_file["sector_code"] = points_file[
                "manufacturing_sector_code"
            ].astype(str) + points_file["mining_sector_code"].astype(str)
            points_file["subsector_code"] = points_file[
                "manufacturing_subsector_code"
            ].astype(str) + points_file["mining_subsector_code"].astype(str)
            points_file.drop(
                [
                    "manufacturing_sector_code",
                    "mining_sector_code",
                    "manufacturing_subsector_code",
                    "mining_subsector_code",
                ],
                axis=1,
                inplace=True,
            )
            ...
            points_file = pd.merge(
                points_file, nsdmb_layers, how="left", on=layer["layer_columns"]
            )

        points_sector_codes = ["X"] + list(
            set(points_file["sector_code"].values.tolist())
        )
        points_file = match_points_to_nearest_buildings(
            buildings_input, points_file, points_sector_codes
        )

        points_file = points_file[points_file["distance"] < distance_buffer]
        for point in points_file.itertuples():
            buildings_input.loc[buildings_input["bid"] == point.bid, "sector_code"] = (
                point.sector_code
            )
            buildings_input.loc[
                buildings_input["bid"] == point.bid, "subsector_code"
            ] = point.subsector_code
            buildings_input.loc[
                buildings_input["bid"] == point.bid, "assigned_point"
            ] = layer["layer_name"]
    ...
    if (
        ...
        buildings_input.to_file(
            os.path.join(
                building_data_path, "buildings_assigned_economic_sectors.gpkg"
            ),
            driver="GPKG",
        )
