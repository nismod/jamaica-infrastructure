...
def join_values(x,column_types):
    ...
    # print (p0,vals)
    if len(vals) > 1:
        return ",".join([v for v in vals if v not in ['','X','nan','None','none','yes','Yes','Unknown']])
    else:
        return vals[0]    
...
def join_values_in_column(x,column_merge):
    ...
    if len(vals) > 1:
        return ",".join([v for v in vals if v not in ['','X','nan','None','none','yes','Yes']])
    else:
        return vals[0]

def create_sector_subsector_attribute_columns(building_dataframe,building_id_column,merge_columns,new_columns,attribute_columns,infra_columns):
    for col in new_columns + attribute_columns + infra_columns:
        building_dataframe[col] = building_dataframe[col].apply(str)
        building_dataframe[col] = building_dataframe[col].replace([r"^\s+$",'','nan','None','none'], 'X', regex=True)
    ...
    building_dataframe['assigned_attribute'] = building_dataframe.progress_apply(
                                            lambda x:join_values(x,attribute_columns),
                                            axis=1)
    building_dataframe['infra_type'] = building_dataframe.progress_apply(
                                            lambda x:join_values(x,infra_columns),
                                            axis=1)
    building_dataframe.drop(new_columns + attribute_columns + infra_columns + ['sector_subsector'],axis=1,inplace=True)
    building_dataframe = building_dataframe.drop_duplicates(subset=[building_id_column],keep='first')

    return building_dataframe

def sector_subsector_asset_mapping(building_dataframe,building_id_column,
    ...
    for column in attribute_columns:
        sector_map = pd.read_excel(
                                    building_sector_mapping_file,
                                    sheet_name=column)
        sector_map = sector_map.drop_duplicates([column,'sector_code','subsector_code','infra_sector'],keep='first')
        sector_map = sector_map[[column,'sector_code','subsector_code','infra_sector']] 
    ...
    return create_sector_subsector_attribute_columns(building_dataframe,building_id_column,
                                        merge_columns,new_columns,
                                        attribute_columns,infra_columns)
...
def identify_specific_areas(x,area_string):
    if area_string in str(x["Classify"]).lower() or area_string in str(x["NAME"]).lower() or area_string in str(x["global_LU_type"]).lower():
        return 1
    else:
        return 0

def column_merging(value_dataframe,building_dataframe,building_id_column,dataframe_merge_columns):
...
def match_buildings_to_landplanning(buildings,gdf,gdf_list):
    matches = gpd.sjoin(buildings[["osm_id","geometry"]],
    ...
    matches = pd.merge(matches, gdf[['layer_id','geometry']],how="left",on=["layer_id"])
...
def match_buildings_to_polygon_dataframe(buildings,gdf,epsg=4326,spatial_join=True):    
    if spatial_join is True:
        ...
        gdf_intersections = gpd.sjoin(gdf,
                                    buildings[['osm_id','geometry']], 
                                    how="inner", predicate='intersects').reset_index()
        df = pd.DataFrame(list(set(gdf_intersections['osm_id'].values.tolist())),columns=['osm_id'])
        ...
        for column in ['sector_code','subsector_code','assigned_attribute','infra_type']:
            ...
            gdf_intersections[column] = gdf_intersections[column].replace([r"^\s+$",'','nan','None','none'], 'X', regex=True)

            df = pd.merge(df,gdf_intersections[['osm_id',
                                        column,
    ...
    buildings = create_sector_subsector_attribute_columns(buildings,'osm_id',
                                                        merge_columns,
                                                        new_columns,
                                                        columns_types,
                                                        infra_columns)
...
def possible_nonbuilding(x,land_use_assigned):
    ...
    for l in land_use_assigned:
        non_building_type += ','.join([a for a in b if str(a).lower() == str(l).lower()])
...
def main(config):
    ...
    residential_min_area = 11.15 # Minimum area of residential dwelling allowed in Jamaica
    ...
    buildings_input = match_buildings_to_polygon_dataframe(buildings_input,
                                                            points_of_interest,
                                                            epsg=epsg_jamaica) 
    matched_buildings.append(buildings_input[buildings_input["sector_code"] != "X"])
    buildings_input = buildings_input[buildings_input["sector_code"] == "X"]
    write_output = True
    if write_output is True: # We can write the intermediary output for sense checking if we want to
        write_df = gpd.GeoDataFrame(pd.concat(matched_buildings + [buildings_input],
                                axis=0,ignore_index=True),
                                geometry="geometry",
                                crs=f"EPSG:{epsg_jamaica}")
        write_df.to_file(os.path.join(
                                    building_data_path,
                                    'buildings_assigned_economic_sectors_intermediate.gpkg'),
                                    layer="points_merged", 
                                    driver='GPKG')
    ...
    land_use_types = gpd.read_file(os.path.join(
                                processed_data_path,
                                "land_type_and_use",
                                "jamaica_land_use_combined.gpkg"),layer="areas")
    ...
    mining_types = [("bauxite","C","132","Bauxite mining and alumina"),("quarry","C","141","Quarry")]
    for i,(mtype,msect,msubsect,mname) in enumerate(mining_types):
        land_use_types[f'{mtype}_identify'] = land_use_types.progress_apply(lambda x: identify_specific_areas(x,mtype),axis=1)
    ...
    commercial_buildings["layer"] = commercial_buildings["layer"].replace("quarries",'', regex=True)
    commercial_buildings['sector_code'] = 'X'
    commercial_buildings['subsector_code'] = 'X'
    
    industry_export_trade_buildings = commercial_buildings[commercial_buildings["layer"].isin(["trade_plants","exporters","industry"])]
    industry_export_trade_ids = list(set(industry_export_trade_buildings["osm_id"].values.tolist()))
    social_buildings = commercial_buildings[~commercial_buildings["osm_id"].isin(industry_export_trade_ids)]
    ...
    for layer_properties in layer_dict:
        commercial_layer = industry_export_trade_buildings[industry_export_trade_buildings["layer"] == layer_properties['layer_name']]
        if layer_properties['layer_name'] == 'industry':
            ...
            for sh in range(len(layer_properties['excel_sheets'])):
                ...
                commercial_layer = column_merging(nsdmb_layers,commercial_layer,"osm_id",layer_properties['layer_columns'][sh])
        else:
            nsdmb_layers = pd.read_excel(os.path.join(
                                    building_data_path,
                                    layer_properties['excel_file']),
            ...
            commercial_layer = column_merging(nsdmb_layers,commercial_layer,"osm_id",layer_properties['layer_columns'])
    ...
    commercial_buildings = commercial_buildings[~commercial_buildings["osm_id"].isin(commercial_layers_ids)]
    ...
    commercial_buildings = commercial_buildings[["osm_id","sector_code","subsector_code","assigned_attribute","infra_type"]]    
    ...
    ports = gpd.read_file(os.path.join(processed_data_path,"networks","transport","port_polygon.gpkg"),layer="areas")
    ports["sector_code"] = "I"
    ...
    ports = gpd.read_file(os.path.join(processed_data_path,"networks","transport","airport_polygon.gpkg"),layer="areas")
    ports["sector_code"] = "I"
    ports["subsector_code"] = "600"
    ...
    land_use_types = gpd.read_file(os.path.join(
                                processed_data_path,
                                "land_type_and_use",
                                "jamaica_land_use_combined_with_sectors.gpkg"),layer="areas")
    ...
    land_use_types = create_sector_subsector_attribute_columns(land_use_types,'layer_id',
    ...
    """
    residential_buildings = gpd.read_file(os.path.join(
                                    incoming_data_path,
                                    "JAM_residential",
                                    "jam_residential.shp"))
    ...
    residential_buildings = residential_buildings[["osm_id","sector_code","subsector_code","assigned_attribute","infra_type"]]
    ...
    if write_output is True: # We can write the intermediary output for sense checking if we want to
        write_df = gpd.GeoDataFrame(pd.concat(matched_buildings + [buildings_input],
                                    ...
                                    geometry="geometry",
                                    crs=f"EPSG:{epsg_jamaica}")
        write_df['area_sqm'] = write_df.progress_apply(lambda x:x.geometry.area,axis=1)
        write_df.to_file(os.path.join(
                                    building_data_path,
                                    'buildings_assigned_economic_sectors_intermediate.gpkg'),
                                  layer="final",
    ...
    known_assignment = gpd.read_file(os.path.join(
                                    building_data_path,
                                    'manual_building_assign.shp')
                                  )
    known_assignment.rename(columns={"sector_cod":"sector_code",
                                    "subsector_":"subsector_code",
                                    "assigned_a":"assigned_attribute"},
                            inplace=True)
    known_assignment = known_assignment[["osm_id","sector_code","subsector_code","assigned_attribute","infra_type"]]
    ...
    write_df = pd.concat(matched_buildings + [buildings_input],
                        axis=0,ignore_index=True)
    ...
    #                     "Waterbody","dump","l"]
    land_use_mapping = ["Under Construction",
                        "Derelict Building","Drain",
                        "Drainage","Dump","Earth Drain",
                        "Inaccessible","Land Cover-Ruinate",
                        "Landfill","No Build Zone","Ruinate",
                        "Under Construction",
    ...
    write_df = gpd.read_file(os.path.join(
                                    building_data_path,
                                    'buildings_assigned_economic_sectors_intermediate.gpkg'),
                                  layer="final"
                                  )

    write_df["remove_type"] = write_df.progress_apply(lambda x:possible_nonbuilding(x,land_use_mapping),axis=1)
    write_df[["to_remove","remove"]] = write_df["remove_type"].apply(pd.Series)
    write_df.drop("remove_type",axis=1,inplace=True)

    write_output = True
    if write_output is True: # We can write the intermediary output for sense checking if we want to
        write_df = gpd.GeoDataFrame(write_df,
                                    geometry="geometry",
                                    crs=f"EPSG:{epsg_jamaica}")
        write_df.to_file(os.path.join(
                                    building_data_path,
    ...
    buildings_input = gpd.read_file(os.path.join(
                                    ...
                                    'buildings_assigned_economic_sectors_intermediate.gpkg'),
                                  layer="final")
    buildings_input["find_aqua"] = buildings_input.progress_apply(lambda x:1 if "B" in x.sector_code else 0,axis=1)
    ...
    fishing_locations = gpd.read_file(os.path.join(processed_data_path,
                                        "land_type_and_use","aqua_farms.gpkg"), 
                        layer="areas")
    fishery_matches = gpd.sjoin(fishing_locations,buildings_input,how="inner", predicate='intersects').reset_index()
    fishing_ids = list(set(fishery_matches["osm_id"].values.tolist()))
    buildings_input.loc[buildings_input["osm_id"].isin(fishing_ids),"sector_code"] = "B"
    buildings_input.loc[buildings_input["osm_id"].isin(fishing_ids),"subsector_code"] = "50"
    buildings_input.loc[buildings_input["osm_id"].isin(fishing_ids),"assigned_attribute"] = "Aqua Farm"


    all_fishing_assigned_ids = [a for a in all_fishing_assigned_ids if a not in fishing_ids]

    buildings_input = buildings_input[~buildings_input["osm_id"].isin(all_fishing_assigned_ids)]
    ...
    assigned_fishing["sector_code"] = assigned_fishing.progress_apply(
                            lambda x:','.join([s for s in str(x.sector_code).split(",") if s != "B"]),axis=1)
    assigned_fishing["subsector_code"] = assigned_fishing.progress_apply(
                            lambda x:','.join([s for s in str(x.subsector_code).split(",") if s not in (50,"50")]),axis=1)

    assigned_fishing.drop("find_aqua",axis=1,inplace=True)
    buildings_input = gpd.GeoDataFrame(pd.concat([buildings_input,assigned_fishing],axis=0,ignore_index=True),
    ...
    for row in buildings_input.itertuples():
        ...
        buildings_sector_classes += list(zip(str(row.sector_code).split(","),str(row.subsector_code).split(",")))

    buildings_attriibutes = pd.DataFrame(buildings_attriibutes,columns=["assigned_attribute"])
    buildings_attriibutes = buildings_attriibutes.value_counts().reset_index(name='counts')
    buildings_attriibutes.to_csv(os.path.join(
                                    building_data_path,
                                    'building_attributes.csv'),
                                  index=False
                                  )
    buildings_sector_classes = pd.DataFrame(buildings_sector_classes,columns=["sector_code","subsector_code"])
    buildings_sector_classes = buildings_sector_classes.value_counts().reset_index(name='counts')
    buildings_sector_classes.to_csv(os.path.join(
                                    building_data_path,
                                    'buidling_sector_subsectors.csv'),
                                  index=False
                                  )

    costs_shapefiles = ["Construction_Permit_Mapping_Jan-March_Q4_2019-2020",
                            "Construction_Permits_October_-_December_(Q3)_2019-2020",
                            "Construction_Permits_Q1_2019_2020",
                            "Q2-2019- Construction Permits"]
    cost_columns = [['Nature_of_', 'Building_T','Parish','B_Area', 'Est_Val'],
    ...
    for c in range(len(costs_shapefiles)):
        ...
        building_costs = gpd.read_file(os.path.join(incoming_data_path,
                                "construction_permits",
                                "Construction Permit Mapping 2019-2020 FY",
                                f"{shapefile}.shp"))[columns]
        building_costs.columns = ["development_type","building_type",'parish',"area_sqm","cost"]
        building_costs = building_costs[(building_costs["area_sqm"] > 0) & (building_costs["cost"] > 0)]
    ...
    cost_ranges = quantiles(all_costs,["development_type","building_type"],["cost_persqm"])
    building_types = ["Commercial",
                        ...
                        "Residential",
                        "Resort"]
    cost_ranges = cost_ranges[(cost_ranges["development_type"] == "Erection") & (cost_ranges["building_type"].isin(building_types))]
    print (cost_ranges)
    save_file = True
    if save_file == True:
        cost_ranges.to_csv(os.path.join(
                                        building_data_path,
                                        'building_cost_ranges.csv'),
                                      index=False
                                      )
    ...
    buildings_input["building_type"] = buildings_input.progress_apply(lambda x:assign_building_type(x),axis=1)
    buildings_input = pd.merge(buildings_input,
                            cost_ranges[["building_type",
                            "min_damage_cost","max_damage_cost",
                            "mean_damage_cost"]],
                    how="left",on=["building_type"])
    ...
    buildings_input["residential_type"] = buildings_input.progress_apply(lambda x: 1 if "RES" in str(x.sector_code) else 0,axis=1)
    residential_buildings = buildings_input[(buildings_input["area_sqm"] >= residential_min_area) & (buildings_input["residential_type"] == 1)]

    population_buildings = gpd.sjoin(population[["ED_ID",population_column,"geometry"]],
                            residential_buildings[["osm_id","sector_code","subsector_code",
                                            "building_type","area_sqm","geometry"]],
                            how="inner", predicate='intersects').reset_index()
    population_buildings.drop("geometry",axis=1,inplace=True)
    pop_sums = population_buildings.groupby(["ED_ID","osm_id"]).agg({"area_sqm": "sum"})
    pop_frac = pop_sums.groupby(level=0).apply(lambda x: x/float(x.sum()))
    pop_frac = pop_frac.reset_index(level=["ED_ID","osm_id"])
    print (pop_frac)
    residential_populations = pd.merge(population_buildings[["ED_ID","osm_id",population_column]],pop_frac,how="left",on=["ED_ID","osm_id"])
    del pop_sums,pop_frac
    residential_populations["residential_population"] = residential_populations[population_column]*residential_populations["area_sqm"]
    residential_populations = residential_populations.round({'residential_population': 0})
    print (residential_populations["residential_population"].sum())
    residential_populations = residential_populations.groupby("osm_id")["residential_population"].sum().reset_index()
    print (residential_populations)
    buildings_input = pd.merge(buildings_input,residential_populations,how="left",on=["osm_id"])
    buildings_input["residential_population"] = buildings_input["residential_population"].fillna(0)
    write_output = True
    if write_output is True: # We can write the intermediary output for sense checking if we want to
        buildings_input.to_file(os.path.join(
                                    building_data_path,
