...
def gdf_geom_clip(gdf_in, clip_geom):
    ...
    filtered dataframe
    """
    return gdf_in.loc[gdf_in['geometry'].apply(lambda x: x.intersects(clip_geom))].reset_index(drop=True)
...
def main(config):
    ...
    hydrobasins_outputs = os.path.join(processed_data_path,"hydrobasins","hydrobasins.gpkg")
        ...
        for file in files:
            if file.endswith(".zip"):
                ...
                with BytesCollection(zipshp.read()) as src:
                    ...
                    basins.to_file(hydrobasins_outputs,layer=file.replace(".zip",""),driver="GPKG")
