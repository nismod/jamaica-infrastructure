...
def get_unique_matches(dataframe):
    ...
    return (
        df,
        dataframe[
            dataframe["edge_id"].isin(df[df["count"] == 1]["edge_id"].values.tolist())
        ],
    )
    ...
    if len(remaining_matches.index) > 0:
        ...
        while len(remaining_matches.index) > 0:
            ...
            filter_remaining_matches = (
                remaining_matches.groupby("edge_id")["fraction_intersection"]
                .max()
                .reset_index()
            )
            ...
            remaining_matches = pd.merge(
                ...
                filter_remaining_matches,
                how="left",
                on=["edge_id", "fraction_intersection"],
            ).fillna(0)
            ...
            remaining_matches = remaining_matches[
                remaining_matches["edge_id"].isin(
                    df1[df1["count"] > 1]["edge_id"].values.tolist()
                )
            ]
...
def main(config):
    ...
    if implement_step is True:
        edges = gpd.read_parquet(
            os.path.join(road_network_path, "different_roads", "edges.geoparquet")
        )
        nodes = gpd.read_parquet(
            os.path.join(road_network_path, "different_roads", "nodes.geoparquet")
        )
        ...
        if save_intermediary_results is True:
            edges.to_file(
                os.path.join(road_network_path, "osm_roads.gpkg"),
                layer="edges",
                driver="GPKG",
            )
            nodes.to_file(
    ...
    implement_step = True
    if implement_step is True:
        nwa_edges = gpd.read_file(
            os.path.join(
                road_network_path, "different_roads", "roads_main_NWA_NSDMD.gpkg"
            )
        )
        ...
        nwa_edges["road_surface"] = nwa_edges.progress_apply(
            lambda x: modify_road_surface(x), axis=1
        )
        nwa_edges["section_name"] = nwa_edges.progress_apply(
            lambda x: modify_road_section(x), axis=1
        )
        nwa_edges["traffic_count"] = nwa_edges.progress_apply(
            lambda x: modify_traffic_count(x), axis=1
        )
        nwa_edges["average_wi"] = nwa_edges.progress_apply(
        ...
        nwa_roads = gpd.read_file(
            os.path.join(road_network_path, "NWA", "main_road_NWA.shp")
        )
        ...
        nwa_roads.rename(
            columns={"section_": "section_name", "street_nam": "street_name"},
            inplace=True,
        )
        nwa_roads = pd.merge(
            nwa_roads,
        ...
        nwa_roads["street_name"] = nwa_roads.progress_apply(
            lambda x: modify_street_name(x), axis=1
        )
        nwa_roads["road_width"] = nwa_roads.progress_apply(
            lambda x: modify_road_width(x), axis=1
        )
        nwa_roads["road_surface"] = nwa_roads.progress_apply(
            lambda x: modify_road_surface_again(x), axis=1
        )
    ...
    if implement_step is True:
        edges = gpd.read_file(
            os.path.join(road_network_path, "osm_roads.gpkg"), layer="edges"
        )
        nodes = gpd.read_file(
            os.path.join(road_network_path, "osm_roads.gpkg"), layer="nodes"
        )
        ...
        if save_intermediary_results is True:
            road_select.to_file(
                store_intersections, layer="selected_roads", driver="GPKG"
            )
        ...
        print(
            ...
            len(list(set(unique_matches["edge_id"].values.tolist()))),
        )
        multiple_matches = road_select[
            road_select["edge_id"].isin(
                df1[df1["count"] > 1]["edge_id"].values.tolist()
            )
        ]
        ...
        closest_matches = (
            multiple_matches.groupby("edge_id")["length_difference"].min().reset_index()
        )
        ...
        print(
            "* Total equal length matches - first pass:",
            len(filter_multiple_matches.index),
        )
        print(
            "* Total equal length ID matches - first pass:",
            len(list(set(filter_multiple_matches["edge_id"].values.tolist()))),
        )

        finished_matches = list(
            set(
                unique_matches["edge_id"].values.tolist()
        ...
        all_matched_pairs = [
            unique_matches[["edge_id", "nwa_edge_id"]],
        ...
        nwa_edges = pd.merge(
            nwa_roads, nwa_matches, how="left", on=["nwa_edge_id"]
        ).fillna(0)
        if save_intermediary_results is True:
        ...
        remaining_matches = road_length_matches_filtering(
            road_select, nwa_roads, edges, 30, 1.01, save_buffer_file=False
        )
        if save_intermediary_results is True:
            remaining_matches.to_file(
                store_intersections,
        ...
        print(
            "* Total ID matches",
            len(list(set(all_matched_pairs["edge_id"].values.tolist()))),
        )
        ...
        if save_intermediary_results is True:
            df = gpd.GeoDataFrame(
                pd.merge(edges, all_matched_pairs, how="left", on=["edge_id"]),
                geometry="geometry",
    ...
    edges["road_surface"] = edges.progress_apply(
        lambda x: remodify_road_surface(x), axis=1
    )
    edges["section_name"] = edges.progress_apply(
        lambda x: remodify_road_section(x), axis=1
    )
    ...
    all_matches["NWA Road Class"] = all_matches.apply(
        lambda x: modify_merged_class(x), axis=1
    )
    ...
    all_matches_class = (
        all_matches.groupby("edge_id")["NWA Road Class"].apply(list).reset_index()
    )
    all_matches_class["NWA Road Class"] = all_matches_class.apply(
        lambda x: "".join(list(set(x["NWA Road Class"]))), axis=1
    )
    ...
    edges["road_class"] = edges.progress_apply(
        lambda x: modify_road_class_with_points(x), axis=1
    )
    ...
    edges["asset_type"] = edges.progress_apply(
        lambda x: create_road_asset_type(x), axis=1
    )
