...
def estimate_costs_and_units(x):
    if x.asset_type == "pipeline":
        min_damage_cost = (
            float(str(x["cost ($J) - lower bound"]).replace(",", ""))
            / x.geometry.length
        )
        max_damage_cost = (
            float(str(x["cost ($J) - upper bound"]).replace(",", ""))
            / x.geometry.length
        )
        cost_unit = "$J/m"
    else:
...
def main(config):
    ...
    nodes = pd.merge(
        nodes, cost_data, left_on="asset_type_cost_data", right_on="asset", how="left"
    )
    nodes["min_damage_cost"] = (
        nodes["cost ($J) - lower bound"].str.replace(",", "").astype(float)
    )
    nodes["max_damage_cost"] = (
        nodes["cost ($J) - upper bound"].str.replace(",", "").astype(float)
    )
    nodes["cost_unit"] = "$J"
    ...
    edges = pd.merge(
        edges, cost_data, left_on="asset_type_cost_data", right_on="asset", how="left"
    )
    edges["cost_and_units"] = edges.progress_apply(
        lambda x: estimate_costs_and_units(x), axis=1
    )
    edges[["cost_unit", "min_damage_cost", "max_damage_cost"]] = edges[
        "cost_and_units"
    ].apply(pd.Series)
