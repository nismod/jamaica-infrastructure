...
def make_grid_points(country_geodata, res=0.25):
    ...
    for i in range(0, len(country_df)):
        lon_min, lon_max, lat_min, lat_max = return_lat_lon_bound(
            country_df.iloc[i].geometry
        )
    ...
    vertical_centroid = np.linspace(
        min_lat_c + delta_vertical / 2,
        max_lat_c - delta_vertical / 2,
        vertical_grid.astype(int),
    )
    horizontal_centroid = np.linspace(
        min_lon_c + delta_horizontal / 2,
        max_lon_c - delta_horizontal / 2,
        horizontal_grid.astype(int),
    )
...
def TC_analysis(lat, lon, name, country, region, sample):
    ...
    TC["number_hur"] = (
        TC["year"].astype(int).astype(str) + "_" + TC["number"].astype(int).astype(str)
    )
    ...
    TC_sample_maximum = (
        ...
        .sort_values(by="wind_location", ascending=False)
        .drop_duplicates(subset=["number_hur"], keep="first")
    )
...
for sample in list_years:
    ...
    output = pool.map(
        TC_analysis,
        grid_jamaica.latitude,
        grid_jamaica.longitude,
        grid_jamaica.ID_point,
        grid_jamaica.country,
        grid_jamaica.region,
        grid_jamaica.sample_num,
    )
    output_files = pd.concat(output)
    output_files.to_csv(
        "Output/TC_Jamaica_" + str(region) + "_" + str(sample) + ".csv", index=False
    )
