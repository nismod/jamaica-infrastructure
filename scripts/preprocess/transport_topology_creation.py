...
def main(config):
    ...
    for sector in sector_details:
        if sector["node_layer"]:
            nodes = geopandas_read_file_type(
                os.path.join(database_path, sector["node_folder"]),
                sector["node_layer"],
        ...
        if sector["edge_layer"]:
            edges = geopandas_read_file_type(
                os.path.join(database_path, sector["edge_folder"]),
                sector["edge_layer"],
                sector["edge_file_type"],
            )
        else:
            print(
                "{} error: there should be at an edge layer".format(
                    sector["sector_name"]
                )
            )
            break
        ...
        create_network_from_nodes_and_edges(
            ...
            sector["sector_name"],
            os.path.join(
                processed_data_path, "networks", f"{sector['sector_name']}.gpkg"
            ),
        )
