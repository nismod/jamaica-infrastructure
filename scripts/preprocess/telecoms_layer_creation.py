...
def match_areas_and_add_values(
    gdf_assign, gdf_values, gdf_assign_ids, gdf_values_ids, gdf_density_column
):
    matches = gpd.sjoin(
        gdf_assign, gdf_values, how="inner", predicate="intersects"
    ).reset_index()
    matches.rename(columns={"geometry": "assign_geometry"}, inplace=True)
    matches = pd.merge(
        matches,
        gdf_values[gdf_values_ids + ["geometry"]],
        how="left",
        on=gdf_values_ids,
    )
    matches["area_match"] = matches.progress_apply(
        lambda x: x["assign_geometry"].intersection(x["geometry"].buffer(0)).area,
        axis=1,
    )
...
def main(config):
    ...
    telecoms_nodes["node_id"] = telecoms_nodes.progress_apply(
        lambda x: f"teln_{x.node_id}", axis=1
    )
    telecoms_nodes = snkit.network.drop_duplicate_geometries(
        telecoms_nodes, keep="first"
    )
    telecoms_nodes = telecoms_nodes.reset_index()

    telecoms_nodes.to_file(
        os.path.join(processed_data_path, "networks", "telecoms", "telecoms.gpkg"),
        layer="nodes",
        driver="GPKG",
    )
    del telecoms_nodes
    telecoms_voronoi = gpd.read_file(
        os.path.join(incoming_data_path, "telecoms", "telecoms_voronoi.gpkg")
    )
    ...
    telecoms_voronoi = match_areas_and_add_values(
        telecoms_voronoi, population_areas, ["node_id"], ["ED_ID", "ED"], "2019"
    )
    telecoms_voronoi.to_file(
        os.path.join(processed_data_path, "networks", "telecoms", "telecoms.gpkg"),
        layer="voronoi",
        driver="GPKG",
    )
