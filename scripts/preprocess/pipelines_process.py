...
def main(config):
    ...
    sewer_data_path = os.path.join(incoming_data_path, "water", "wastewater", "raw")
    #### assumption list
    # potable pipes and sewers are same cost

    # # read in raw files
    pot_pipelines = gpd.read_file(
        os.path.join(potable_data_path, "pipelines_network_NWC.shp")
    ...
    pipelines["Diameter"] = np.where(
        pipelines["Diameter"] < 0, 2, pipelines["Diameter"]
    )
    ...
    pipelines["min_damage_cost"] = (
        pipelines["cost ($J) - lower bound"].str.replace(",", "").astype(float)
        * pipelines["Diameter"]
    )
    pipelines["max_damage_cost"] = (
        pipelines["cost ($J) - upper bound"].str.replace(",", "").astype(float)
        * pipelines["Diameter"]
    )
    ...
    # provide id
    pipelines["edge_id"] = pipelines.apply(
        lambda edge: f"{edge.asset_type}_{edge.OBJECTID}", axis=1
    )
    # # export as gpkg
    pipelines = gpd.GeoDataFrame(
        pipelines, crs=f"EPSG:{epsg_jamaica}", geometry=pipelines["geometry"]
    )
    pipelines = pipelines[pipelines["Length"] > 0]
    # export as gpkg
    fname = os.path.join(processed_data_path, "networks", "water", "pipelines_NWC.gpkg")
