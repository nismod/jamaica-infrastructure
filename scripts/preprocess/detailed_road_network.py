...
def modify_road_surface(x):
    ...
    if (x["constructi"]) or (x["construc_1"]):
        if x["constructi"] == x["construc_1"]:
            return x["constructi"]
        elif (x["constructi"] in ["SD & AC", "AC & SD"]) and (
            x["construc_1"] in ["SD & AC", "AC & SD"]
        ):
            return "SD & AC"
        elif x["construc_1"]:
            return x["construc_1"]
...
def match_roads(
    ...
    edge_dataframe,
    geom_buffer=10,
    fraction_intersection=0.95,
    length_intersected=100,
    save_buffer_file=False,
):
    # print (nwa_edges)
    buffer_dataframe["geometry"] = buffer_dataframe.geometry.progress_apply(
        lambda x: x.buffer(geom_buffer)
    )

    # Save the result to sense check by visual inspection on QGIS. Not a necessary step
    if save_buffer_file is not False:
        buffer_dataframe.to_file(
            save_buffer_file, layer=f"nwa_buffer_{geom_buffer}", driver="GPKG"
        )

    road_matches = gpd.sjoin(
        edge_dataframe,
        buffer_dataframe[["nwa_edge_id", "geometry"]],
        how="inner",
        op="intersects",
    ).reset_index()
    if len(road_matches.index) > 0:
        buffer_dataframe.rename(columns={"geometry": "nwa_geometry"}, inplace=True)
        road_matches = pd.merge(
            road_matches,
            buffer_dataframe[["nwa_edge_id", "nwa_geometry", "nwa_length"]],
            how="left",
            on=["nwa_edge_id"],
        )
        ...
        road_matches["length_intersected"] = road_matches.progress_apply(
            lambda x: (x.geometry.intersection(x.nwa_geometry).length), axis=1
        )
...
def get_unique_matches(dataframe):
    ...
    return (
        df,
        dataframe[dataframe["id"].isin(df[df["count"] == 1]["id"].values.tolist())],
    )


def road_length_matches_filtering(
    remaining_matches,
    buffer_dataframe,
    edge_dataframe,
    geom_buffer,
    ...
    if len(remaining_matches.index) > 0:
        ...
        while len(remaining_matches.index) > 0:
            ...
            filter_remaining_matches = (
                remaining_matches.groupby("id")["fraction_intersection"]
                .max()
                .reset_index()
            )
            filter_remaining_matches["longest_section_match"] = 1
            remaining_matches = pd.merge(
                remaining_matches,
                filter_remaining_matches,
                how="left",
                on=["id", "fraction_intersection"],
            ).fillna(0)
            remaining_matches = remaining_matches[
                remaining_matches["longest_section_match"] == 1
            ]
            ...
            remaining_matches = remaining_matches[
                remaining_matches["id"].isin(
                    df1[df1["count"] > 1]["id"].values.tolist()
                )
            ]
            ...
            if len(remaining_matches.index) > 0:
                ...
                remaining_matches = match_roads(
                    buffer_dataframe[
                        buffer_dataframe["nwa_edge_id"].isin(nwa_ids)
                    ].copy(),
...
def main(config):
    ...
    nwa_roads = gpd.read_file(
        os.path.join(road_network_path, "nwa_roads.gpkg"), layer="edges"
    )
    nwa_roads.set_crs(epsg=epsg_jamaica)
    nwa_roads["nwa_length"] = nwa_roads.progress_apply(
    ...
    save_intermediary_results = (
        True  # If you want to store the intermediary results for sense checking
    )
    ...
    if save_intermediary_results is True:
        unique_matches.to_file(
            store_intersections, layer="unique_matches", driver="GPKG"
        )

    print("* Total unique matches - first pass:", len(unique_matches.index))
    print(
        "* Total unique ID matches - first pass:",
        len(list(set(unique_matches["id"].values.tolist()))),
    )
    multiple_matches = road_select[
        road_select["id"].isin(df1[df1["count"] > 1]["id"].values.tolist())
    ]
    multiple_matches["length_difference"] = multiple_matches.progress_apply(
        lambda x: abs(x["fraction_intersection"] - x["fraction_buffer"]), axis=1
    )
    closest_matches = (
        multiple_matches.groupby("id")["length_difference"].min().reset_index()
    )
    closest_matches = closest_matches[closest_matches["length_difference"] <= 1e-2]
    closest_matches["closeness"] = 1
    multiple_matches = pd.merge(
        multiple_matches, closest_matches, how="left", on=["id", "length_difference"]
    ).fillna(0)
    filter_multiple_matches = multiple_matches[multiple_matches["closeness"] == 1]
    if save_intermediary_results is True:
        filter_multiple_matches.to_file(
            store_intersections, layer="multiple_matches_filter", driver="GPKG"
        )

    print(
        "* Total equal length matches - first pass:", len(filter_multiple_matches.index)
    )
    print(
        "* Total equal length ID matches - first pass:",
        len(list(set(filter_multiple_matches["id"].values.tolist()))),
    )

    finished_matches = list(
        set(
            unique_matches["id"].values.tolist()
            + filter_multiple_matches["id"].values.tolist()
        )
    )
    remaining_matches = road_select[~road_select["id"].isin(finished_matches)]
    remaining_matches = road_length_matches_filtering(
        remaining_matches, nwa_roads, edges, 10, 1.5, save_buffer_file=False
    )

    if save_intermediary_results is True:
        remaining_matches.to_file(
            store_intersections, layer="length_matches_filter", driver="GPKG"
        )

    print(
        "* Total longest length of intersection matches - first pass:",
        len(remaining_matches.index),
    )
    print(
        "* Total longest length of intersection matches ID matches - first pass:",
        len(list(set(remaining_matches["id"].values.tolist()))),
    )

    all_matched_pairs = [
        unique_matches[["id", "nwa_edge_id"]],
    ...
    nwa_matches = pd.concat(
        [
            unique_matches[nwa_column_check],
    ...
    nwa_matches = (
        nwa_matches.groupby("nwa_edge_id")["fraction_buffer"].sum().reset_index()
    )
    nwa_edges = pd.merge(nwa_roads, nwa_matches, how="left", on=["nwa_edge_id"]).fillna(
        0
    )
    if save_intermediary_results is True:
        nwa_edges[nwa_edges["fraction_buffer"] <= 0.5].to_file(
            store_intersections, layer="nwa_remaining_matches", driver="GPKG"
        )

    nwa_edges = nwa_edges[nwa_edges["fraction_buffer"] <= 0.5]
    match_edges = edges[~edges["id"].isin(all_matches)]
    match_edges = match_edges[
        ~match_edges["_7"].isin(["OTHER", "TRACK"])
    ]  # There are a lot of unwanted edges if we do not filter out this case
    road_select = match_roads(
        nwa_edges,
        match_edges,
        geom_buffer=120,
    ...
    if save_intermediary_results is True:
        road_select.to_file(
            store_intersections, layer="wider_buffer_match", driver="GPKG"
        )

    remaining_matches = road_length_matches_filtering(
        road_select, nwa_roads, edges, 120, 1.01, save_buffer_file=False
    )
    if save_intermediary_results is True:
        remaining_matches.to_file(
            store_intersections,
            layer="length_matches_filter_wider_buffer",
            driver="GPKG",
        )

    print(
        "* Total longest length of intersection matches - second pass:",
        len(remaining_matches.index),
    )
    print(
        "* Total longest length of intersection matches ID matches - second pass:",
        len(list(set(remaining_matches["id"].values.tolist()))),
    )
    ...
    all_matched_pairs = pd.merge(
        all_matched_pairs, nwa_roads[nwa_columns], how="left", on=["nwa_edge_id"]
    )
    ...
    edges.to_file(
        os.path.join(road_network_path, "roads.gpkg"),
        layer="edges_modified_intermediate",
        driver="GPKG",
    )
    ...
    edges["section_name"] = edges.progress_apply(
        lambda x: modify_road_section(x), axis=1
    )
    ...
    edges["traffic_count"] = edges.progress_apply(
        lambda x: modify_traffic_count(x), axis=1
    )
    ...
    traffic_points = traffic_points[
        ["NWA Section No.", "Total_IN", "X Coordinate", "Y Coordinate"]
    ]
    traffic_points["point_id"] = traffic_points.index.values.tolist()
    geometry = [
        Point(xy)
        for xy in zip(traffic_points["X Coordinate"], traffic_points["Y Coordinate"])
    ]
    traffic_points = gpd.GeoDataFrame(
        traffic_points, crs={"init": "epsg:4326"}, geometry=geometry
    )
    ...
    edges_buffered["geometry"] = edges_buffered.geometry.progress_apply(
        lambda x: x.buffer(geom_buffer)
    )
    point_matches = gpd.sjoin(
        traffic_points,
        edges_buffered[["edge_id", "section_name", "geometry"]],
        how="inner",
        op="intersects",
    ).reset_index()

    point_matches.rename(columns={"geometry": "point_geometry"}, inplace=True)
    point_matches = pd.merge(
        point_matches, edges[["edge_id", "geometry"]], how="left", on=["edge_id"]
    )
    point_matches["distance"] = point_matches.progress_apply(
        lambda x: x.geometry.distance(x.point_geometry), axis=1
    )
    # point_matches.to_csv(os.path.join(incoming_data_path,'roads','nwa_traffic_counts','matched_roads.csv'),index=False)

    all_matches = []
    same_matches = point_matches[
        point_matches["NWA Section No."] == point_matches["section_name"]
    ]
    nearest_points = same_matches.groupby("point_id")["distance"].min().reset_index()
    nearest_points = pd.merge(
        nearest_points,
        same_matches[
            [
                "point_id",
    ...
    remaining_matches = point_matches[
        ~point_matches["point_id"].isin(
            list(set(nearest_points["point_id"].values.tolist()))
        )
    ]
    nearest_points = (
        remaining_matches.groupby("point_id")["distance"].min().reset_index()
    )
    nearest_points = pd.merge(
        nearest_points,
        remaining_matches[
    ...
    all_matches["NWA Section No."] = all_matches.apply(
        lambda x: modify_merged_sections(x), axis=1
    )

    all_matches_totals = all_matches.groupby("edge_id")["Total_IN"].mean().reset_index()
    all_matches_sections = (
        all_matches.groupby("edge_id")["NWA Section No."].apply(list).reset_index()
    )
    all_matches_sections["NWA Section No."] = all_matches_sections.apply(
        lambda x: ",".join(list(set(x["NWA Section No."]))), axis=1
    )

    all_matches = pd.merge(
        all_matches_sections, all_matches_totals, how="left", on=["edge_id"]
    )
    edges = pd.merge(edges, all_matches, how="left", on=["edge_id"])
    edges["section_name"] = edges.progress_apply(
        lambda x: modify_road_section_with_points(x), axis=1
    )
    edges["traffic_count"] = edges.progress_apply(
        lambda x: modify_traffic_count_with_points(x), axis=1
    )
    ...
    nodes["asset_type"] = nodes.progress_apply(
        lambda x: assign_node_asset_type(x), axis=1
    )
    nodes.to_file(
        os.path.join(processed_data_path, "networks", "transport", "roads.gpkg"),
        layer="nodes",
    ...
    network_file = os.path.join(
        processed_data_path, "networks", "transport", "roads.gpkg"
    )
    ...
    max_id = max(
        [int(v.split("_")[1]) for v in component_edges["edge_id"].values.tolist()]
    )
    ...
    component_edges["link_intersection"] = component_edges.progress_apply(
        lambda x: (
            1 if x.geometry.intersects(edges_links.geometry.values[0]) is True else 0
        ),
        axis=1,
    )
    ...
    largest_component_distances = ckdnearest(
        component_nodes[component_nodes["component"] == 0][
            ["id", "component", "geometry"]
        ],
        component_nodes[component_nodes["component"] == 1][
            ["id", "component", "geometry"]
        ],
    )
    remaining_component_distances = ckdnearest(
        remaining_componets[["id", "component", "geometry"]],
        largest_components[["id", "component", "geometry"]],
    )
    distances = pd.concat(
        [largest_component_distances, remaining_component_distances],
        axis=0,
        ignore_index=True,
    )
    ...
    distances = pd.merge(
        distances, component_nodes[["id", "geometry"]], how="left", on=["id"]
    )
    ...
    combined_network = gpd.GeoDataFrame(
        combined_network, geometry="geometry", crs=f"EPSG:{epsg_jamaica}"
    )
    ...
    component_nodes[["id", "geometry"]].to_file(
        os.path.join(component_file), layer="nodes", driver="GPKG"
    )
    edges_components = assign_network_components.main(
        component_file, component_file, "id"
    )

    roads_connected_edges = gpd.read_file(
        os.path.join(
            processed_data_path,
            "networks",
            "transport",
    ...
    roads_connected_nodes = gpd.read_file(
        os.path.join(
            processed_data_path,
            "networks",
            "transport",
    ...
    roads_connected_edges = pd.merge(
        roads_connected_edges,
        roads_edges,
        how="left",
        on=["edge_id", "from_node", "to_node"],
    )
    print(roads_connected_edges)
    roads_connected_nodes = pd.merge(
        roads_connected_nodes, roads_nodes, how="left", on=["id"]
    )
    ...
    unassigned_roads["road_length_m"] = unassigned_roads.progress_apply(
        lambda x: x.geometry.length, axis=1
    )
    ...
    unassigned_columns = [
        c for c in roads_edges.columns.values.tolist() if c not in assigned_columns
    ]
    reassigned_roads = []
    for row in unassigned_roads.itertuples():
        ...
        for col in unassigned_columns:
            ...
            values = list(
                set(from_values[col].values.tolist() + to_values[col].values.tolist())
            )
            if len(values) == 1:
    ...
    reassigned_roads = pd.merge(
        reassigned_roads, unassigned_roads[assigned_columns], how="left", on=["edge_id"]
    )
    print(reassigned_roads)
    roads = gpd.GeoDataFrame(
        pd.concat([assigned_roads, reassigned_roads], axis=0, ignore_index=True),
        geometry="geometry",
    ...
    edges.drop(
        ["min_reopen_cost", "mean_reopen_cost", "max_reopen_cost"], axis=1, inplace=True
    )
    edges.to_file(
        os.path.join(processed_data_path, "networks", "transport", "roads.gpkg"),
        layer="edges",
    ...
    nodes = gpd.GeoDataFrame(
        pd.concat([bridges, non_bridges], axis=0, ignore_index=True),
        geometry="geometry",
        crs=f"EPSG:{epsg_jamaica}",
    )
    print(nodes)
