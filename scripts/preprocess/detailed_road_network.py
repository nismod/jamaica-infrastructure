...
def main(config):
    ...
    road_edges_dir = os.path.join(
        database_path, "National Land Agency Datasets", "PIOJ", "Street Centerline"
    )
    roads = []
    ...
    roads = gpd.GeoDataFrame(
        pd.concat(roads, ignore_index=True, sort="False", axis=0),
        geometry="geometry",
        crs={"init": f"epsg:{epsg_jamaica}"},
    )
    ...
    roads.to_file(
        os.path.join(os.path.join(database_path, "pioj_roads", "detailed_roads.shp"))
    )
    # print (roads)

    bridges = gpd.read_file(
        os.path.join(database_path, "NWA Bridges 2016", "NWA Bridges 2016.shp")
    )
    ...
    create_network_from_nodes_and_edges(
        ...
        "road",
        os.path.join(
            processed_data_path, "networks", "transport", "detailed_roads.gpkg"
        ),
    )
