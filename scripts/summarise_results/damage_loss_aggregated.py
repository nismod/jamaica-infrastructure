...
def main(config,damages_losses_folder,
    ...
    for asset_info in asset_data_details.itertuples():
        ...
        if os.path.isfile(os.path.join(damages_losses_results,
                        f"{asset_info.asset_gpkg}_{asset_info.asset_layer}_damages.parquet")) is True:
            damages = pd.read_parquet(os.path.join(damages_losses_results,
                        f"{asset_info.asset_gpkg}_{asset_info.asset_layer}_damages.parquet"))
            haz_columns = [c for c in damages.columns.values.tolist() if c not in [asset_id,"damage_cost_unit"]]
            df = pd.DataFrame()
        ...
        if os.path.isfile(os.path.join(damages_losses_results,
                        f"{asset_info.asset_gpkg}_{asset_info.asset_layer}_losses.parquet")) is True:
            losses = pd.read_parquet(os.path.join(damages_losses_results,
                        f"{asset_info.asset_gpkg}_{asset_info.asset_layer}_losses.parquet"))
            haz_columns = [c for c in losses.columns.values.tolist() if c not in [asset_id,"economic_loss_unit"]]
            df = pd.DataFrame()
            df["sector"] = [asset_info.sector]
            df["asset_gpkg"] = [asset_info.asset_gpkg]
            df["asset_layer"] = [asset_info.asset_layer]
    ...
    if len(damages_totals) > 0:
        ...
        damages.to_csv(os.path.join(summary_results,
                    f"sector_total_damages.csv"),index=False)
        del damages
    ...
    if len(loss_totals) > 0:
        ...
        losses.to_csv(os.path.join(summary_results,
                    f"sector_level_losses.csv"),index=False)
        del losses   
