...
def main(config):
    ...
    asset_summary_results = os.path.join(
        output_data_path, "asset_attributes_summarised"
    )
    ...
    asset_data_details = pd.read_csv(
        os.path.join(processed_data_path, "networks", "network_layers_details.csv")
    )
    ...
    for asset_info in asset_data_details.itertuples():
        ...
        for group_index in asset_group_indexes:
            group_indexes = [group_index] + getattr(
                asset_info, asset_cost_unit_column
            ).split(",")
            ...
            # print (asset_groups)
            asset_groups = dict(
                [(k, gr) for k, gr in asset_groups.items() if gr != "none"]
            )
            # print (asset_groups)
            ...
            asset_sums = ["count"]
            if (
                asset_info.asset_layer == "edges"
                or asset_info.asset_layer == "edges_v1.1"
            ):
            ...
            min_groups = [
                item
                for sublist in [
                    gr.split(",") for v, gr in asset_groups.items() if "max_" not in v
                ]
                for item in sublist
            ]
            max_groups = [
                item
                for sublist in [
                    gr.split(",") for v, gr in asset_groups.items() if "min_" not in v
                ]
                for item in sublist
            ]
            ...
            min_df.rename(
                columns=dict([(v, f"min_{v}") for v in min_groups if "min_" not in v]),
                inplace=True,
            )
            # min_groups = [f"min_{v}" for v in min_df. if "min_" not in v]
            max_df = asset_df.groupby(group_indexes)[max_groups].max().reset_index()
            max_df.rename(
                columns=dict([(v, f"max_{v}") for v in max_groups if "max_" not in v]),
                inplace=True,
            )
            # max_groups = [f"max_{v}" for v in max_groups if "max_" not in v]

            min_max_df = pd.merge(sum_df, min_df, how="left", on=group_indexes).fillna(
                0
            )
            min_max_df = pd.merge(
                min_max_df, max_df, how="left", on=group_indexes
            ).fillna(0)
            # print (min_max_df.columns)
            if "length" in min_max_df.columns.values.tolist():
                print(
                    f"Total kms of {asset_info.asset_gpkg} {asset_info.asset_layer} {min_max_df.length.values.sum()}"
                )
            for c in [
                v for v in min_max_df.columns.values.tolist() if v not in group_indexes
            ]:
                # print (c)
                min_max_df[c] = min_max_df[c].apply(
                    lambda v: f"{int(v):,}" if v > 10 else f"{round(v,2):,}"
                )
