...
from jamaica_sector_attributes import (
...
def filter_damaged_assets(
    sector, damage_data_path, damage_string, asset_dataframe, layer_key
):
...
def get_damage_columns(damage_dictionary, damage_dataframe):
    ...
    for d in dc:
        damage_sum_columns += [
            c for c in damage_dataframe.columns.values.tolist() if f"{d}_" in c
        ]
...
def damages_grouped(
    ...
    damages = (
        damages.groupby(damage_groupby + ["sector", "subsector"], dropna=False)
    ...
    damages.columns = [
        str(c).replace("undefended", "").replace("designed_protection", "")
...
def main(config):
    ...
    damage_cases = [
        ...
        # },
        {
            ...
            "damage_data_path": os.path.join(
                output_data_path, "direct_damages_summary"
            ),
    ...
    for dg_cs in damage_cases:
        ...
        csv_outputs = os.path.join(
            summary_results, f"hazard_rcp_epoch_{dg_cs['damage_type']}_totals.csv"
        )
        for sector in sector_attributes:
            ...
            if len(edges) > 0:
                edges_damages = filter_damaged_assets(
                    sector, damage_data_path, damage_string, edges, "edge"
                )
            ...
            if sector["sector_label"] == "Potable water":
                ...
                nodes = get_sector_layer(sector, asset_data_path, "node")
            if len(nodes) > 0:
                nodes_damages = filter_damaged_assets(
                    sector, damage_data_path, damage_string, nodes, "node"
                )
            ...
            if len(areas) > 0:
                areas_damages = filter_damaged_assets(
                    sector, damage_data_path, damage_string, areas, "area"
                )
        ...
        damages_combined = pd.concat(
            damages_combined, axis=0, ignore_index=True
        ).fillna(0)
    ...
    damage_data_path = os.path.join(output_data_path, "loss_damage_timeseries")
    csv_outputs = os.path.join(summary_results, "hazard_rcp_timeseries_totals.csv")
