...
def curve_interpolation(x_curve, y_curve, x_new):
    interpolate_values = interp1d(
        x_curve, y_curve, fill_value=(min(y_curve), max(y_curve)), bounds_error=False
    )
    return interpolate_values(x_new)


def expected_risks_pivot(
    v, probabilites, probability_threshold, flood_protection_column
):
    """Calculate expected risks"""
    prob_risk = sorted(
        [(p, getattr(v, str(p))) for p in probabilites], key=lambda x: x[0]
    )
...
def risks_pivot(
    ...
    df = (
        dataframe.set_index(index_columns)
        .pivot(columns=probability_column)[risk_column]
        .reset_index()
        .rename_axis(None, axis=1)
    ).fillna(0)
    df.columns = df.columns.astype(str)
    df[expected_risk_column] = df.progress_apply(
        lambda x: expected_risks_pivot(
            x, probabilites, probability_threshold, flood_protection_column
        ),
        axis=1,
    )
...
def calculate_discounting_arrays(
    discount_rate=4.5,
    growth_rate=5.0,
    start_year=2020,
    end_year=2050,
