...
def main(config):
    ...
    telecoms["T_GDP"] = (
        output * telecoms[f"{population_year}"] / telecoms[f"{population_year}"].sum()
    )
    ...
    telecoms_nodes = pd.merge(
        telecoms_nodes, telecoms[[telecoms_id, "T_GDP"]], how="left", on=[telecoms_id]
    )
    ...
    telecoms_buildings = match_buildings_to_footprints(
        buildings, telecoms[[telecoms_id, "geometry"]], building_id, telecoms_id
    )
    telecoms_buildings = telecoms_buildings[[telecoms_id, building_id] + gdp_columns]
    telecoms_buildings.to_csv(
        os.path.join(
            processed_data_path,
    ...
    telecoms_buildings = (
        telecoms_buildings.groupby([telecoms_id])[gdp_columns].sum().reset_index()
    )
    telecoms_buildings = pd.merge(
        telecoms[[telecoms_id, "T_GDP"]],
        telecoms_buildings,
        how="left",
        on=[telecoms_id],
    ).fillna(0)
    telecoms_buildings["total_GDP"] = telecoms_buildings[["T_GDP"] + gdp_columns].sum(
        axis=1
    )
