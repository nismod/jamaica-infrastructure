...
def create_network_from_nodes_and_edges(
    nodes, edges, node_edge_prefix, out_fname, by=None
):
...
def network_od_paths_assembly(
    points_dataframe, graph, cost_criteria, flow_value_column
):
    ...
    save_paths_df = save_paths_df[
        (save_paths_df[flow_value_column] > 0) & (save_paths_df["origin_id"] != 0)
    ]
