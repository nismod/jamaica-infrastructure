...
def modify_working_ages(working_dataframe):
    ...
    return pd.DataFrame(
        ...
        columns=[
            "age_group",
            "percent_employed_min",
            "percent_employed_mean",
            "percent_employed_max",
        ],
    )
...
def modify_population_age_change(population_dataframe):
    ...
    for row in population_dataframe.itertuples():
        if "-" not in row.age_group:
            ...
            df.append(
                ("95-", row.growth_rate_min, row.growth_rate_mean, row.growth_rate_max)
            )
        else:
            df.append(
                (
    ...
    return pd.DataFrame(
        df,
        columns=["age_group", "growth_rate_min", "growth_rate_mean", "growth_rate_max"],
    )
...
def get_population_changes(population_dataframe, start_year, end_year, base_year):
    ...
    for year in range(start_year, end_year):
        ...
        population_dataframe[year_column] = population_dataframe.progress_apply(
            lambda x: int(str(x[year_column]).replace(",", "")), axis=1
        )
        population_dataframe[f"growth_rate_{year}"] = (
            np.power(
                population_dataframe[year_column]
                / population_dataframe[base_year_column],
                1 / (year - base_year),
            )
            - 1
        )
    ...
    population_dataframe["growth_rate_mean"] = population_dataframe[
        growth_rate_columns
    ].mean(axis=1)
    population_dataframe["growth_rate_min"] = population_dataframe[
        growth_rate_columns
    ].min(axis=1)
    population_dataframe["growth_rate_max"] = population_dataframe[
        growth_rate_columns
    ].max(axis=1)
...
def main(config):
    ...
    male_nonworking_columns = [
        c for c in male_age_wise_columns if c not in male_working_columns
    ]
    ...
    female_nonworking_columns = [
        c for c in female_age_wise_columns if c not in female_working_columns
    ]
    ...
    population_growth["parish_rename"] = population_growth.progress_apply(
    ...
    parish_population_change = get_population_changes(
        parish_population_change, 2014, 2020, baseyear
    )
    ...
    employed_population = employed_population[
        employed_population["age_group"] != "TOTAL"
    ]
    working_categories = [
        ("Male", "male_age_groups.csv", "MLE"),
        ("Female", "female_age_groups.csv", "FMLE"),
    ]
    pop_totals = []
    all_year_columns = []
    for i, (gender, gender_file, gender_string) in enumerate(working_categories):
        ...
        gender_working["age_bins"] = gender_working.progress_apply(
            lambda x: get_age_bins(x), axis=1
        )
        gender_working[["min_age", "max_age"]] = gender_working["age_bins"].apply(
            pd.Series
        )
        ...
        for forecast in forecasts:
            ...
            for pop in population_changes.itertuples():
                ...
                population_gender[year_column] += (
                    ...
                    * 0.01
                    * getattr(pop, f"percent_employed_{forecast}")
                )

            population_gender[year_column] = population_gender.apply(
                lambda x: round(x[str(year_column)], 0), axis=1
            )

        population_gender.drop(
            male_age_wise_columns
            + female_age_wise_columns
            + ["parish_rename", "population"],
            axis=1,
            inplace=True,
        )
        pop_totals.append(
            population_gender[
                [
                    c
                    for c in population_gender.columns.values.tolist()
                    if c != "geometry"
                ]
            ]
        )
    ...
    index_cols = [
        c for c in pop_totals[0].columns.values.tolist() if c not in all_year_columns
    ]
    pop_totals = pd.merge(pop_totals[0], pop_totals[1], how="left", on=index_cols)
    for forecast in forecasts:
        pop_totals[f"{forecast}_working_{baseyear}"] = (
            pop_totals[f"{forecast}_male_{baseyear}"]
            + pop_totals[f"{forecast}_female_{baseyear}"]
        )
    ...
    """
    population_estimation = pd.merge(
        population_estimation,
        parish_population_change[
            ["parish_rename", "growth_rate_min", "growth_rate_mean", "growth_rate_max"]
        ],
        how="left",
        on=["parish_rename"],
    )
    ...
    for forecast in forecasts:
        population_estimation["working_frac"] = population_estimation.apply(
            lambda x: (
                x[f"{forecast}_working_{baseyear}"] / x["2011"] if x["2011"] > 0 else 0
            ),
            axis=1,
        )
        ...
        population_estimation.rename(
            columns={f"{forecast}_working_{baseyear}": f"working_{baseyear}"},
            inplace=True,
        )
        for year in range(2012, 2100):
            population_estimation[str(year)] = population_estimation[
                str(baseyear)
            ] * np.power(
                1 + population_estimation[f"growth_rate_{forecast}"], year - baseyear
            )
            population_estimation[f"working_{year}"] = (
                population_estimation["working_frac"] * population_estimation[str(year)]
            )
            population_estimation[str(year)] = population_estimation.apply(
                lambda x: round(x[str(year)], 0), axis=1
            )
            population_estimation[f"working_{year}"] = population_estimation.apply(
                lambda x: round(x[f"working_{year}"], 0), axis=1
            )

        population_estimation.drop("working_frac", axis=1, inplace=True)
        result_gdf = population_estimation[
            [
                c
                for c in population_estimation.columns.values.tolist()
                if c not in exclude_columns
            ]
        ]
        result_gdf.to_file(
            os.path.join(
                processed_data_path, "population", "population_projections.gpkg"
            ),
            layer=forecast,
            driver="GPKG",
        )
