...
@click.option(
    "--processed-data-dir", "-p", required=True, type=click.Path(dir_okay=True, file_okay=False, exists=True),
    help="Designated processed data directory.",
)
@click.option(
    "--results-dir", "-r", required=True, type=click.Path(dir_okay=True, file_okay=False, exists=True),
...
def main(*, processed_data_dir, results_dir):
    ...
    all_failures = []
    bridge_failures = []
    for root, dirs, files in os.walk(os.path.join(transport_failures_dir, "scenario_results")):
        for file in files:
    ...
    labour_flows = gpd.read_file(
        os.path.join(results_dir, "flow_mapping", "labour_trips_and_activity.pq"),
        layer="edges",
    )
    ...
    output_dir = os.path.join(
        results_dir, "economic_losses", "single_failure_scenarios"
    )
    all_failures.to_csv(
        os.path.join(
    ...
    bridge_failures.to_csv(
        os.path.join(
            output_dir, "single_point_failure_road_bridges_economic_losses.csv"
        ),
        index=False,
    )
    ...
    ports = pd.merge(ports[["node_id"]], od_losses, how="left", on=["node_id"]).fillna(
        0
    )
    ...
    rail_losses["rerouting_loss"] = (1 - rail_losses["no_access"]) * (
        rail_losses["trade_rerouting_loss"]
    )
    rail_losses["isolation_loss"] = rail_losses["no_access"] * rail_losses["trade_loss"]
    rail_losses["economic_loss"] = (
        rail_losses["rerouting_loss"] + rail_losses["isolation_loss"]
    )

    rail_losses = (
        rail_losses.groupby(["node_id"])[
            ["rerouting_loss", "isolation_loss", "economic_loss"]
        ]
        .sum()
        .reset_index()
    ...
    rail_losses.to_csv(
        os.path.join(
            output_dir, "single_point_failure_rail_stations_economic_losses.csv"
        ),
        index=False,
    )
