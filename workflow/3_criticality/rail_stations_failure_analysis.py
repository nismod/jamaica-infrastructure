...
@click.option(
    "--edges-file", "-e", required=True, type=click.Path(exists=True, dir_okay=False, readable=True),
    help="Jamaican Multimodal Network GeoPackage file"
)
@click.option(
    "--rail-nodes-file", "-r", required=True, type=click.Path(exists=True, dir_okay=False, readable=True),
...
def main(*, flow_data_dir, edges_file, rail_nodes_file, output_path):
    ...
    losses = (
        edge_fail_results.groupby(["node_id", "no_access"])[
            "time_loss",
    ...
    rerouting_times_min = (
        edge_fail_results.groupby(["node_id", "no_access"])["time_loss"]
        .min()
        .reset_index()
    )
    ...
    losses = pd.merge(
        losses, rerouting_times_min, how="left", on=["node_id", "no_access"]
    )
    losses = pd.merge(
        losses, rerouting_times_max, how="left", on=["node_id", "no_access"]
    )
    losses = pd.merge(
        losses, rerouting_times_mean, how="left", on=["node_id", "no_access"]
    )
