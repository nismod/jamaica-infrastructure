...
def coastal_protection_aggregate(
    ...
    for row in network.itertuples():
        ...
        EAD_EAEL_path, = filter(lambda x: Path(x).name == f"coastal_{gpkg_layer}_adaptation_costs_avoided_EAD_EAEL.csv", EAD_EAEL_paths)
        adaptation_cost_path, = filter(lambda x: Path(x).name == f"{gpkg_layer}_adaptation_timeseries_and_npvs.csv", adaptation_cost_paths)
        lookup_path, = filter(lambda x: Path(x).name == f"{gpkg_layer}_coastal_filtered.parquet", lookup_paths)

        by_layer.append(collate_costs_and_avoided_risks(gpkg_layer, row.asset_id_column, EAD_EAEL_path, adaptation_cost_path, lookup_path))
    ...
    per_segment_costs = per_segment_costs[~per_segment_costs["adapt_cost_npv"].isna()] \
        .sort_values("id") \
    ...
    for rcp in sorted(rcps):
        for var in ("amin", "mean", "amax"):
            BCR[f"flooding__rcp_{rcp}__avoided_risk_{var}"] = \
                BCR[f"flooding__rcp_{rcp}__avoided_EAD_{var}"] \
                + BCR[f"flooding__rcp_{rcp}__avoided_EAEL_{var}"]
