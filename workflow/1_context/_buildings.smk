...
rule building_height_assignments:
    ...
    input:
        ...
        # vector_raster_intersection_script = "scripts/preprocess/vector_raster_intersections.py",
        building_layers = f"{DATA}/buildings/building_layers.csv",
        building_rasters = f"{DATA}/buildings/building_rasters.csv",
        buildings_econ_activity = f"{OUTPUT}/buildings/buildings_assigned_economic_activity.gpkg"
    output:
        results = f"{OUTPUT}/building_heights/buildings_heights.gpkg"
    shell:
        """
        python {input.script} \
            --data-dir {DATA} \
            --output-dir {OUTPUT} \
            --building_layers {input.building_layers} \
            --building_rasters {input.building_rasters} \
            --buildings_econ_activity {input.buildings_econ_activity} 
        """
