...
def get_sector_gdp(
    ...
    if output > 0:
        ...
        sector_df[f"{sector_code}_t_ij_ext"] = (
            sector_df[f"t_ij_ext"] * sector_df[sector_code]
        )
...
@click.option(
    ...
    required=True,
    type=int,
    help="Financial year for GDP data",
)
@click.option(
    "--buffer-distance",
    "-bd",
    required=True,
    type=int,
    help="Buffer distance in meters for population analysis",
)
@click.option(
    "--pop-year",
    "-py",
...
@click.option(
    ...
    help="Path to population projections gpkg",
)
@click.option(
    ...
    required=True,
    type=click.Path(exists=True, dir_okay=False, file_okay=True, readable=True),
    help="Path to buildings economic activity gpkg",
)
@click.option(
    "--econ-output",
    "-eo",
    required=True,
    type=click.Path(exists=True, dir_okay=False, file_okay=True, readable=True),
    help="Path to economic output xlsx",
)
@click.option(
    "--fishing-locals",
    "-fl",
...
@click.option(
    ...
    required=True,
    type=click.Path(exists=True, dir_okay=False, file_okay=True, readable=True),
    help="Path to agriculture buildings GDP csv",
)
@click.option(
    "--mining-buildings-gdp",
    "-mbg",
    required=True,
    type=click.Path(exists=True, dir_okay=False, file_okay=True, readable=True),
    help="Path to mining buildings GDP csv",
)
def main(
    data_dir,
    incoming_data_dir,
    ...
    population_buffer["geometry"] = population_buffer.apply(
    ...
    within_distance["t_ij_ext"] = within_distance.progress_apply(
        lambda x: x[f"working_{population_year}"]
    ...
    if write_results is True:
        region_attractiveness.to_csv(
            os.path.join(
                processed_data_path, "population", "region_attractiveness.csv"
            ),
            index=False,
        )
        within_distance.to_csv(
            os.path.join(processed_data_path, "population", "radiation_flows.csv"),
            index=False,
        )
    ...
    if estimate_radiation is True:
        ...
        region_radiation.rename(
            columns={"from_ED_ID": "ED_ID", "from_ED": "ED"}, inplace=True
        )
        region_radiation.to_csv(
            os.path.join(processed_data_path, "population", "region_radiation.csv"),
            index=False,
        )
        region_radiation = pd.merge(
            region_radiation,
    ...
    # This is just part of some testing of the resutls. Can be skipped
    if write_results is True:
        region_attractiveness.to_file(
            os.path.join(processed_data_path, "population", "region_radiation.gpkg"),
            layer="attractiveness_index",
            driver="GPKG",
        )
    ...
    for scode in sector_codes:
        ...
        if scode in ("D", "H", "J", "K", "L"):
            ...
            agri_buildings = pd.read_csv(agri_buildings_gdp)
            ...
            mining_buildings = pd.read_csv(mining_buildings_gdp)
