...
def create_polygons_from_clusters(labels, shape, transform, crs, df):
    ...
    for cluster_label in np.unique(labels):
        ...
        for contour in contours:
            polygon_coords = [transform * (col, row) for row, col in contour]
...
def order_edges_around_island(edge_layer):
    ...
    edges_by_from_id = {edge["from_id"]: edge for _, edge in edge_layer.iterrows()}
...
# Adapted and modified from https://github.com/thomas-fred/jam-coastal-protection
def combine_floods_within_scape(
    voronoi_polygons: gpd.GeoDataFrame, flood_polygons: gpd.GeoDataFrame
) -> gpd.GeoDataFrame:
...
def create_voronoi(jamaica_polygon, jam_contour, coastline_polygons, intersections, length_limit, coastline):
    ...
    def hierarchical_subdivision_strategy(
        clipped_gdf,
        centroid_gdf,
        coastline_polygons,
        coastline,
    ...
    def constraint_based_refinement(
        clipped_gdf,
        centroid_gdf,
        coastline_polygons,
        coastline,
        ...
        while True:
            ...
            logging.info(
                f"   Iteration {iteration_count}: Max length = {current_max_length:.1f}, "
    ...
    def apply_aggressive_subdivision(
        clipped_gdf,
        centroid_gdf,
        coastline_polygons,
        coastline,
    ...
    def apply_standard_refinement_step(
        clipped_gdf,
        centroid_gdf,
        coastline_polygons,
        coastline,
    ...
    def apply_final_aggressive_subdivision(
        clipped_gdf,
        centroid_gdf,
        coastline_polygons,
        coastline,
...
# Adapted and modified from https://github.com/thomas-fred/jam-coastal-protection
def combine_floods_within_voronoi(
    voronoi_polygons: gpd.GeoDataFrame,
    flood_polygons: gpd.GeoDataFrame,
    voronoi_points: gpd.GeoDataFrame,
    pts_id_field: str,
    max_distance: float,
) -> gpd.GeoDataFrame:
    """
...
def linestring_intersect_polygons(
    default_inland_distance: float,
    ...
    for idx, poly in polygons.iterrows():
        ...
        if linestring.geometry.iloc[0].intersects(poly.geometry):
            ...
            initial_intersections.append({"geometry": intersection, "id": poly["id"]})
