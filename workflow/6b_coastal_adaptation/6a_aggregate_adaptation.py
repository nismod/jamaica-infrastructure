...
def collate_costs_and_avoided_risks(
    ...
    per_layer_EAD_EAEL = (
        pd.read_csv(EAD_EAEL_path)
        .drop(columns=["adapt_cost_npv"])
...
def coastal_protection_aggregate(
    ...
    for row in network.itertuples():
        ...
        (EAD_EAEL_path,) = filter(
            ...
            == f"coastal_{gpkg_layer}_adaptation_costs_avoided_EAD_EAEL.csv",
            EAD_EAEL_paths,
        )
        (adaptation_cost_path,) = filter(
            lambda x: Path(x).name
            == f"{gpkg_layer}_adaptation_timeseries_and_npvs.csv",
            adaptation_cost_paths,
        )
        (lookup_path,) = filter(
    ...
    for col in data.columns:
        if match := re.match(
            r"coastal__rcp_(\d\.\d)__avoided_(EAD|EAEL)_(amin|mean|amax)", col
        ):
    ...
    per_segment_costs = (
        per_segment_costs[~per_segment_costs["adapt_cost_npv"].isna()]
        .sort_values("id")
